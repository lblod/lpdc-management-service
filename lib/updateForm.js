...
async function mutate(mutationType, statements, sessionUri = null) {
  ...
  if (parsedStatements.length > 0) {
    if(mutationType == 'DELETE') {
      ...
      // If allowed, we revert to a virtuoso query executing the query according to a SPARQL expression we KNOW works for virtuoso
      // And then trick mu-auth to still send deltas (which will always be by design so)
      ...
      for(const statement of source ) {
        ...
        `;
        // There is no way we can trick mu-auth changing the expression
        // in something that's acceptable for virtuoso. So we call virtuoso directly
        await updateSudo(queryStr, {}, { sparqlEndpoint: 'http://virtuoso:8890/sparql'});
        // And then, to ensure deltas we do the same mu-auth call (discussed with aad)
