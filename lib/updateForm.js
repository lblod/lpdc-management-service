...
export async function updateForm(data, sessionUri) {
    ...
    const result = await update(`
                    WITH ${sparqlEscapeUri(APPLICATION_GRAPH)}
                    DELETE {
                        ${[...deletes].join('\n')}
                        ${modifiedTimestamp.subject.toNT()} ${modifiedTimestamp.predicate.toNT()} ?modifiedTimestamp
                    }
                    INSERT {
                        ${[...inserts, updatedModifiedTimestamp.toNT()].join('\n')}
                    }
                    WHERE {
                        VALUES ?modifiedTimestamp {${modifiedTimestamp.object.toNT()}}
                        ${modifiedTimestamp.subject.toNT()} ${modifiedTimestamp.predicate.toNT()} ?modifiedTimestamp
                    }`);

    const updateResult = result.results.bindings[0]['callret-0'].value;

    if (updateResult.includes("Delete 0 (or less) quads -- done")) {
...
async function mutate(mutationType, statements, sessionUri = null) {
    ...
    const parsedStatements = parseStatements(statements);
