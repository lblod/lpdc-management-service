...
import { sparqlEscapeString, sparqlEscapeDateTime, sparqlEscapeUri, sparqlEscape } from 'mu';
import { FORM_STATUS_CONCEPT } from '../config';
...
//TODO: this should be written in to the correct graph
...
function generateFormTriples(publicServiceUri) {
  const formTypes = ['content', 'translation', 'characteristics'];

  const triples = formTypes.map(type => {
    const formId = uuid();
    ...
    return (
      `
        ...
        skos:prefLabel ${sparqlEscapeString(type)}.
      `
    )
  });
