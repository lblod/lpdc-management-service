...
import {
    ...
    loadCosts,
    loadEvidences,
    loadFinancialAdvantages, loadFormalInformalChoice,
...
import {findConceptOrInstanceLanguages, getChosenForm, selectLanguageVersionForChosenForm} from "./formalInformalChoice";
...
export async function createForm(conceptId, bestuurseenheid) {
    ...
    // FIXME: a concept can not hava a contactPoint this is unnecessary
    ...
    // FIXME: attachments are not used any more and IPDC should not send them to us.
...
function keepOnlyChosenLanguageVersion(publicServiceUri, allTriples, chosenForm) {
    const languageVersionsInConcept = findConceptOrInstanceLanguages(allTriples);
    const languageVersionToKeep = selectLanguageVersionForChosenForm(languageVersionsInConcept, chosenForm);
    const fields = getFieldsWithLanguage(publicServiceUri, allTriples);
    let triples = allTriples;
    for (const field of fields) {
        const triplesForField = allTriples.filter(triple => triple.s.value === field.subject && triple.p.value === field.predicate);
        if (triplesForField.length) {
            ...
            tripleToKeep.o['xml:lang'] = chosenForm === 'informal' ? 'nl-be-x-informal' : 'nl-be-x-formal';
