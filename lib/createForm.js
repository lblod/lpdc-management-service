...
export async function createForm(conceptId, bestuurseenheid) {
  ...
  // Note: mind the avoidance of the bind statement here.
  //  Virtuoso spits weird errors combining construct, bind and optional.
  const contactPointsAddressQuery = `
    ${PREFIXES}
    CONSTRUCT {
      ?address a locn:Address.
      ?address adres:Straatnaam ?street.
      ?address <https://data.vlaanderen.be/ns/adres#Adresvoorstelling.huisnummer> ?houseNumber.
      ?address <https://data.vlaanderen.be/ns/adres#Adresvoorstelling.busnummer> ?mailbox.
      ?address adres:postcode ?postCode.
      ?address adres:gemeentenaam ?municipality.
      ?address adres:land ?country.
    }
    FROM ${sparqlEscapeUri(APPLICATION_GRAPH)}
      WHERE {
        ${sparqlEscapeUri(conceptUri)} a lpdcExt:ConceptualPublicService;
        ...
        OPTIONAL { ?contact lpdcExt:address ?address. }
        OPTIONAL { ?address adres:Straatnaam ?street. }
        OPTIONAL { ?address <https://data.vlaanderen.be/ns/adres#Adresvoorstelling.huisnummer> ?houseNumber. }
        OPTIONAL { ?address <https://data.vlaanderen.be/ns/adres#Adresvoorstelling.busnummer> ?mailbox. }
  ...
  const contactPointAddressData = await queryAndCopySubjects(contactPointsAddressQuery, 'http://data.lblod.info/id/address/');

  const contactPointsQuery = `
    ${PREFIXES}
    CONSTRUCT {
         ?contact a schema:ContactPoint.
         ?contact lpdcExt:address ?address.
  ...
  const contactPointData = await queryAndCopySubjects(contactPointsQuery, 'http://data.lblod.info/id/contact-point/');
