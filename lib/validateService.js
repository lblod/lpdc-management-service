...
async function validateAddresses(serviceUri) {
    ...
    if (addresses.length) {
        const addressIds = [...new Set(addresses.map(triple => triple.s.value))]
        const addressValidationPromises = addressIds.map(async addressId => {
            const gemeente = addresses.find(triple => triple.s.value === addressId && triple.p.value === 'https://data.vlaanderen.be/ns/adres#gemeentenaam')?.o?.value;
            const straat = addresses.find(triple => triple.s.value === addressId && triple.p.value === 'https://data.vlaanderen.be/ns/adres#Straatnaam')?.o?.value;
            const huisnummer = addresses.find(triple => triple.s.value === addressId && triple.p.value === 'https://data.vlaanderen.be/ns/adres#Adresvoorstelling.huisnummer')?.o?.value;
            const busnummer = addresses.find(triple => triple.s.value === addressId && triple.p.value === 'https://data.vlaanderen.be/ns/adres#Adresvoorstelling.busnummer')?.o?.value;
            if (gemeente && straat && huisnummer) {
                const match = await findAddressMatch(gemeente, straat, huisnummer, busnummer);
                return !!match.adressenRegisterId;
            } else {
                return false;
            }
        });
        const addressValidation = await Promise.all(addressValidationPromises);
