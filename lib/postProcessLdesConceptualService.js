...
async function existingCode(code, codeList = 'dvcs:IPDCOrganisaties') {
  const queryStr = `
  ${PREFIXES}
  ASK {
    GRAPH ?g {
...
async function insertCodeListData(codeListData, codeList = 'dvcs:IPDCOrganisaties') {
  codeListData.uuid = uuid();
  const queryStr = `
    ${PREFIXES}
    INSERT {
      ${sparqlEscapeUri(codeListData.uri)} a skos:Concept.
      ${sparqlEscapeUri(codeListData.uri)} skos:inScheme ${codeList}.
      ${sparqlEscapeUri(codeListData.uri)} skos:topConceptOf ${codeList}.
...
async function handleCodeList(code, conceptScheme = 'dvcs:IPDCOrganisaties') {
  const alreadyExists = await existingCode(code, conceptScheme);
  if (!alreadyExists){
    ...
    if (codeListData.prefLabel){
      ...
      await insertCodeListData(codeListData, conceptScheme);
    }
  }
}
