...
async function updateConceptualService( versionedServiceGraph, versionedServiceUri, serviceUri, serviceId ) {
  ...
  const loadAndPostProcess = async (callBack, newType = null) => {
    ...
    for(const bindings of Object.values(bindingsPerSubject)) {
      ...
      finalResults.push(updatedBindings);
    }
    return finalResults;
  };
  ...
  results.push(await loadAndPostProcess(loadOnlineProcedureRules, 'http://data.europa.eu/m8g/Evidence'));
  ...
  const serviceResultBindings = await loadPublicService(versionedServiceUri, { graph, sudo });
  addUuidForSubject(serviceResultBindings, serviceId);
