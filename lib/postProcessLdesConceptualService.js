...
async function updateConceptualService( versionedServiceUri, serviceUri, serviceId ) {
  //TODO: this used to be a big INSERT query, but it seems virtuoso doesn't like too long queries
  //  hence the weird batched update.
  const insertBlocks = [];
  ...
  const queryTemplate = insertBlock =>  `
  ${PREFIXES}
  INSERT {
   GRAPH ${sparqlEscapeUri(CONCEPTUAL_SERVICE_GRAPH)} {
  ...
  for(const insertBlock of insertBlocks) {
    await updateSudo(queryTemplate(insertBlock));
  }
}
