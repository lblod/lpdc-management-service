...
async function removeConceptualService( serviceId ) {
  ...
  await setStatus("deleted", serviceUri);
}
...
async function updateConceptualService( versionedServiceGraph, versionedServiceUri, serviceUri, serviceId ) {
  ...
  await setStatus("updated", versionedServiceUri);
}
...
async function setStatus(status, conceptUri){
  
  let statusUri = "";
  if(status==='updated'){
    statusUri="<http://lblod.data.gift/concepts/5a3168e2-f39b-4b5d-8638-29f935023c83>."
  }
  else if(status==='deleted'){
    statusUri="<http://lblod.data.gift/concepts/cf22e8d1-23c3-45da-89bc-00826eaf23c3>."
  }
  else{
    return;
  }
  ...
  const queryStr=`
    ...
    INSERT{
      GRAPH ?g {
        ?service ext:reviewStatus ${sparqlEscapeUri(statusUri)}
      }
    }
    DELETE{
      GRAPH ?g {
    ...
    WHERE{
      GRAPH ?g {
        ?service dct:source ${sparqlEscapeUri(conceptUri)}.
      }
    }
  `
  try{
