...
import {adjustConceptFormForFormalInformalChoice} from "./adjustConceptFormForFormalInformalChoice";
...
export async function retrieveForm(publicServiceId, formId) {
  ...
  if (isConceptualPublicService) {
    const conceptLanguages = sourceBindings
    ...
    const chosenForm = formalInformalChoice
        .find(triple => triple.p.value === 'https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#chosenForm')
        ?.o?.value;

    form = adjustConceptFormForFormalInformalChoice(form, chosenForm, conceptLanguages);
  }
