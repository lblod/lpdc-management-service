...
import {
  ...
  loadPublicService,
  loadAttachments,
  serviceUriForId, loadFormalInformalChoice
} from './commonQueries';
import {
  adjustConceptFormForFormalInformalChoice,
  findConceptOrInstanceLanguages,
  getChosenForm,
  selectLanguageVersionForChosenForm
...
export async function retrieveForm(publicServiceId, formId) {
  ...
  if (isConceptualPublicService) {
    const conceptLanguages = findConceptOrInstanceLanguages(sourceBindings);
    const chosenForm = getChosenForm(await loadFormalInformalChoice());
    const languageForChosenForm = selectLanguageVersionForChosenForm(conceptLanguages, chosenForm);
    form = adjustLanguageOfForm(form, languageForChosenForm);
  } else {
    const language = findConceptOrInstanceLanguages(sourceBindings)[0];
    form = adjustLanguageOfForm(form, language);
  }
...
export function adjustLanguageOfForm(form, newLanguage) {
  return form.replaceAll(`form:language "nl"`, `form:language "${newLanguage}"`);
}
