...
export class ConceptSparqlRepository implements ConceptRepository {
  ...
  async findById(id: Iri): Promise<Concept> {
    ...
    const mapper = new QuadsToDomainMapper(
      quads,
      new Iri(CONCEPT_GRAPH),
      new LoggingDoubleQuadReporter(new Logger("Concept-QuadsToDomainLogger")),
    );
  ...
  async save(concept: Concept): Promise<void> {
    const triples = new DomainToQuadsMapper(new Iri(CONCEPT_GRAPH))
  ...
  async update(concept: Concept, old: Concept): Promise<void> {
    const oldTriples = new DomainToQuadsMapper(new Iri(CONCEPT_GRAPH))
      .conceptToQuads(old)
      .map((s) => s.toNT());
    const newTriples = new DomainToQuadsMapper(new Iri(CONCEPT_GRAPH))
      .conceptToQuads(concept)
      .map((s) => s.toNT());
