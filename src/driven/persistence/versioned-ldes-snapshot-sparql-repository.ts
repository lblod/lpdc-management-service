...
export class VersionedLdesSnapshotSparqlRepository
  implements VersionedLdesSnapshotRepository
{
  ...
  async findToProcessSnapshots(
    snapshotType: SnapshotType,
  ): Promise<{ snapshotGraph: Iri; snapshotId: Iri }[]> {
    const query = `
  ...
  async addToSuccessfullyProcessedSnapshots(
    snapshotGraph: Iri,
    snapshotId: Iri,
  ): Promise<void> {
    const markerId = new Iri(
      `http://data.lblod.info/id/versioned-ldes-snapshot-processed-marker/${uuid()}`,
    );
  ...
  async addToFailedProcessedSnapshots(
    snapshotGraph: Iri,
    snapshotId: Iri,
    errorMessage: string,
  ): Promise<void> {
  ...
  async hasNewerProcessedSnapshot(
    snapshotGraph: Iri,
