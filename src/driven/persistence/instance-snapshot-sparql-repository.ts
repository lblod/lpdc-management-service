...
export class InstanceSnapshotSparqlRepository
  implements InstanceSnapshotRepository
{
  ...
  protected doubleQuadReporter: DoubleQuadReporter =
    new LoggingDoubleQuadReporter(
      new Logger("InstanceSnapshot-QuadsToDomainLogger"),
    );
  ...
  errorIfNoInstanceSnapshotGraph(instanceSnapshotGraph: Iri): void {
    if (
      !instanceSnapshotGraph.value.startsWith(INSTANCE_SNAPHOT_LDES_GRAPH())
    ) {
      throw new SystemError(
        `Kan Instance Snapshots niet opzoeken in graph <${instanceSnapshotGraph.value}>`,
      );
    }
  }
}
