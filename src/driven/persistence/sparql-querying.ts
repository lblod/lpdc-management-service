...
export class SparqlQuerying {
  ...
  public async deleteInsert(
    query: string,
    resultVerification?: (deleteInsertResults: string[]) => void,
  ): Promise<void> {
    const result = await updateSudo(
      query,
      {},
        ...
        if (bindings) {
          if (bindings.length > 1) {
            throw new SystemError(
              `Expecting a single row from query (${query}), got ${bindings.length} results.`,
            );
          }
        }
        return bindings[0];
  ...
  private asNamedNodeOrLiteral(term: any): NamedNode | Literal {
    ...
    throw new SystemError(
      `Could not parse ${JSON.stringify(term)} as Named Node Or Literal`,
    );
  }
  ...
  private verifyResultToMatch(
    query: string,
    result: any,
    expectedPattern: RegExp,
  ) {
    const results = result.results.bindings.map((b) => b["callret-0"].value);
    if (!results.every((ir) => expectedPattern.test(ir))) {
