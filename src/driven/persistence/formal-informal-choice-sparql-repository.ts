...
export class FormalInformalChoiceSparqlRepository
  implements FormalInformalChoiceRepository
{
  ...
  async findByBestuurseenheid(
    ...
    const formalInformalChoiceId = new Iri(
      result["formalInformalChoiceId"].value,
    );

    const rawChosenForm = result["chosenForm"].value;

    const chosenFormKey: string | undefined = Object.keys(ChosenFormType).find(
      (key) => ChosenFormType[key] === rawChosenForm,
    );
    if (!chosenFormKey) {
      throw new SystemError(
        `Kan '${rawChosenForm}'niet mappen voor Iri: <${formalInformalChoiceId}>`,
      );
    }
