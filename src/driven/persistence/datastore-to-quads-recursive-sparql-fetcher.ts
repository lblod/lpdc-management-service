...
export class DatastoreToQuadsRecursiveSparqlFetcher {
  ...
  async fetch(
    graph: Iri,
    from: Iri,
    predicatesToNotQuery: string[],
    predicatesToStopRecursion: string[],
    illegalTypesToRecurseInto: string[],
  ): Promise<Quad[]> {
  ...
  private async doFetch(
    ...
    const filterQueryFragment =
      predicatesToNotQuery.length > 0
        ? `FILTER (${predicatesToNotQuery.map((predicateId) => `?p != ${sparqlEscapeUri(predicateId)}`).join(" && ")})`
        : "";
      ...
      .forEach((q) => {
        if (illegalTypesToRecurseInto.includes(q.object.value)) {
          throw new SystemError(
            `Recursie in <${q.object.value}> vanuit <${q.subject.value}> is niet toegestaan`,
          );
        }
      });

    const referencedIds = quads
      .filter((q) => {
        return (
          isNamedNode(q.object) &&
