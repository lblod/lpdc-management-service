...
export class LoggingDoubleQuadReporter implements DoubleQuadReporter {
  ...
  report(
    ...
    this._logger.log(
      `DoubleQuad|${graph}|${subject}|${predicate}|${object}|${expectedCount}|${actualCount}|${triples.join("|")}`,
    );
  }
}
...
class StoreAccess {
  ...
  constructor(
    quads: Quad[],
    graphId: Iri,
    doubleQuadReporter: DoubleQuadReporter,
  ) {
    this.store = graph();
  ...
  public statements(
    ...
    options: {
      ...
      o?: any;
    } = { validateUniqueLanguages: true, o: null },
  ): Statement[] {
    const result = this.store.statementsMatching(
      s,
      p,
      options?.o,
      this.graphId,
    ...
    if (options?.validateUniqueLanguages) {
      ...
      const maxLanguageIncidenceOfAnyLanguage = Math.max(
        ...Object.values(languageIncidences),
      );
      if (maxLanguageIncidenceOfAnyLanguage > 1) {
        this.doubleQuadReporter.report(
          this.graphId.value,
          s.value,
          p.value,
          options?.o?.value,
          1,
          maxLanguageIncidenceOfAnyLanguage,
          result.map((r) => r?.object?.toNT()),
        );
      }
    }
  ...
  public uniqueValue(s: NamedNode | BlankNode, p: NamedNode): string | void {
    return this.uniqueStatement(s, p)?.object?.value;
  }

  public uniqueStatement(
    s: NamedNode | BlankNode,
    p: NamedNode,
    o: NamedNode | BlankNode = null,
  ): Statement | undefined {
    ...
    if (allStatementsMatching.length > 1) {
      this.doubleQuadReporter.report(
        ...
        o?.value,
        1,
        allStatementsMatching.length,
        allStatementsMatching.map((r) => r?.object?.toNT()),
      );
    }
...
export class QuadsToDomainMapper {
  ...
  conceptSnapshot(id: Iri): ConceptSnapshot {
    this.errorIfMissingOrIncorrectType(
      id,
    ...
    return new ConceptSnapshot(
      ...
      this.requirements(id)
        .filter(
          (req) => req.title !== undefined && req.description != undefined,
        )
        .map((req) =>
          RequirementBuilder.from(req)
            .withEvidence(
              req.evidence?.title !== undefined &&
                req.evidence?.description !== undefined
                ? req.evidence
                : undefined,
            )
            .build(),
      ...
      this.procedures(id)
        .filter(
          (proc) => proc.title !== undefined && proc.description != undefined,
        )
        .map((proc) =>
      ...
      this.costs(id).filter(
        (c) => c.title !== undefined && c.description !== undefined,
      ),
      this.financialAdvantages(id).filter(
        (fa) => fa.title !== undefined && fa.description !== undefined,
      ),
      this.isVersionOf(id),
      this.dateCreated(id),
  ...
  concept(id: Iri): Concept {
    this.errorIfMissingOrIncorrectType(
      id,
      NS.lpdcExt("ConceptualPublicService"),
    );
  ...
  errorIfMissingOrIncorrectType(id: Iri, type: NamedNode) {
    const typeFoundForId: string = this.storeAccess.uniqueValue(
      this.asNamedOrBlankNode(id),
      NS.rdf("type"),
    );
    if (!typeFoundForId) {
      throw new NotFoundError(
        `Kan <${id}> niet vinden voor type ${type} in graph ${this.graphId}`,
      );
    }
    if (type.value !== typeFoundForId) {
      throw new NotFoundError(
        `Kan <${id}> niet vinden voor type ${type}, maar wel gevonden met type <${typeFoundForId}> in graph ${this.graphId}`,
      );
    }
  }
  ...
  title(id: Iri): LanguageString | undefined {
    return this.asLanguageString(
      this.storeAccess.statements(this.asNamedOrBlankNode(id), NS.dct("title")),
    );
  }

  description(id: Iri): LanguageString | undefined {
    return this.asLanguageString(
      this.storeAccess.statements(
        this.asNamedOrBlankNode(id),
        NS.dct("description"),
      ),
    );
  ...
  keywords(id: Iri): LanguageString[] {
    return this.storeAccess
      .statements(this.asNamedOrBlankNode(id), NS.dcat("keyword"), {
        validateUniqueLanguages: false,
      })
      .map((s) => [s])
  ...
  private isConceptArchived(id: Iri): boolean {
    return !!this.storeAccess.uniqueStatement(
      this.asNamedOrBlankNode(id),
      NS.adms("status"),
  ...
  private instanceReviewStatusType(
    id: Iri,
  ): InstanceReviewStatusType | undefined {
    return this.asEnum(
      InstanceReviewStatusType,
      NS.concepts.reviewStatus,
      this.storeAccess.uniqueStatement(
        this.asNamedOrBlankNode(id),
        NS.ext("reviewStatus"),
      ),
  ...
  costs(id: Iri): Cost[] {
    ...
    costIds.forEach((costId) =>
      this.errorIfMissingOrIncorrectType(costId, NS.m8g("Cost")),
    );

    const costs = costIds.map((costId) =>
      Cost.reconstitute(
        costId,
        this.uuid(costId),
        this.title(costId),
        this.description(costId),
        this.order(costId),
      ),
    );
  ...
  financialAdvantages(id: Iri): FinancialAdvantage[] {
    const financialAdvantageIds = this.asIris(
      this.storeAccess.statements(
        this.asNamedOrBlankNode(id),
        NS.cpsv("produces"),
      ),
    ...
    financialAdvantageIds.forEach((financialAdvantageId) =>
      this.errorIfMissingOrIncorrectType(
        financialAdvantageId,
        NS.lpdcExt("FinancialAdvantage"),
      ),
    );

    const financialAdvantages = financialAdvantageIds.map(
      (financialAdvantageId) =>
        FinancialAdvantage.reconstitute(
          financialAdvantageId,
          this.uuid(financialAdvantageId),
          this.title(financialAdvantageId),
          this.description(financialAdvantageId),
          this.order(financialAdvantageId),
        ),
    );
  ...
  contactPoints(id: Iri): ContactPoint[] {
    const contactPointIds = this.asIris(
      this.storeAccess.statements(
        this.asNamedOrBlankNode(id),
    ...
    const contactPoints: ContactPoint[] = contactPointIds.map(
      (contactPointId) => {
        return ContactPoint.reconstitute(
          contactPointId,
          this.uuid(contactPointId),
          this.url(contactPointId),
          this.email(contactPointId),
          this.telephone(contactPointId),
          this.openingHours(contactPointId),
          this.order(contactPointId),
          this.address(contactPointId),
        );
      },
    );
  ...
  websites(id: Iri, predicate: NamedNode = NS.rdfs("seeAlso")): Website[] {
    const websiteIds = this.asIris(
      this.storeAccess.statements(this.asNamedOrBlankNode(id), predicate),
    );
    ...
    const websites = websiteIds.map((websiteId) =>
      Website.reconstitute(
        websiteId,
        this.uuid(websiteId),
        this.title(websiteId),
        this.description(websiteId),
        this.order(websiteId),
        this.url(websiteId),
      ),
    );
  ...
  procedures(id: Iri): Procedure[] {
    const procedureIds = this.asIris(
      this.storeAccess.statements(
        this.asNamedOrBlankNode(id),
        NS.cpsv("follows"),
      ),
    ...
    const procedures = procedureIds.map((procedureId) =>
      Procedure.reconstitute(
        procedureId,
        this.uuid(procedureId),
        this.title(procedureId),
        this.description(procedureId),
        this.order(procedureId),
        this.websites(procedureId, NS.lpdcExt("hasWebsite")),
      ),
    );
  ...
  requirements(id: Iri): Requirement[] {
    const requirementIds = this.asIris(
      this.storeAccess.statements(
        this.asNamedOrBlankNode(id),
  ...
  private evidence(id: Iri): Evidence | undefined {
    ...
    return Evidence.reconstitute(
      evidenceIds[0],
      this.uuid(evidenceIds[0]),
      this.title(evidenceIds[0]),
      this.description(evidenceIds[0]),
    );
  }

  legalResources(id: Iri): LegalResource[] {
    const legalResourceIds = this.asIris(
      this.storeAccess.statements(
        this.asNamedOrBlankNode(id),
    ...
    const legalResources = legalResourceIds.map((legalResourceId) =>
  ...
  private order(id: Iri): number | undefined {
    return this.asNumber(
      this.storeAccess.uniqueValue(this.asNamedOrBlankNode(id), NS.sh("order")),
    );
  }
  ...
  private asFormatPreservingDate(
    aValue: string | undefined,
  ): FormatPreservingDate | undefined {
  ...
  private asEnums<T>(
    ...
    return statements.map((statement) =>
      this.asEnum(enumObj, namespace, statement),
    );
  }
  ...
  private asEnum<T>(
    ...
    for (const key in enumObj) {
      if (
        ...
        (namespace as Namespace)(enumObj[key] as string).value ===
  ...
  private asLanguageString(
    ...
    const languageString = LanguageString.of(
      ...
      literals?.find((l) => l.language?.toLowerCase() === Language.FORMAL)
        ?.value,
      literals?.find((l) => l.language?.toLowerCase() === Language.INFORMAL)
        ?.value,
      literals?.find(
        (l) => l.language?.toLowerCase() === Language.GENERATED_FORMAL,
      )?.value,
      literals?.find(
        (l) => l.language?.toLowerCase() === Language.GENERATED_INFORMAL,
      )?.value,
    );
  ...
  private asLiteral(statement: Statement): Literal {
    if (!isLiteral(statement.object)) {
      throw new SystemError(
        `Expecting (${statement}) to have an object that is a literal.`,
      );
    }
    return statement.object as Literal;
  }

  private objectAsNamedNodeOrBlankNode(
    statement: Statement | undefined,
  ): NamedNode | BlankNode {
    if (!statement) {
      return undefined;
    }
    if (!isNamedNode(statement.object) && !isBlankNode(statement.object)) {
      throw new SystemError(
        `Expecting (${statement}) to have an object that is a named node or a blank node.`,
      );
    }
  ...
  private sort(anArray: any) {
    ...
    const orders = anArray.map((obj: { id: any }) => {
      const id = obj.id;
      const order: number | undefined = this.asNumber(
        this.storeAccess.uniqueValue(
          this.asNamedOrBlankNode(id),
          NS.sh("order"),
        ),
