...
export class SelectConceptLanguageDomainService {
  public selectAvailableLanguageUsingFormalInformalChoice(
    conceptOrConceptSnapshot: Concept | ConceptSnapshot,
    formalInformalChoice: FormalInformalChoice | undefined,
  ): Language {
    return this.selectAvailableLanguage(
      conceptOrConceptSnapshot,
      formalInformalChoice?.chosenForm === ChosenFormType.INFORMAL,
    );
  }

  public selectAvailableLanguage(
    conceptOrConceptSnapshot: Concept | ConceptSnapshot,
    informalLanguageVariantRequired: boolean,
  ): Language {
    ...
    if (informalLanguageVariantRequired) {
      ...
      if (conceptLanguages.includes(Language.FORMAL)) {
        ...
        conceptLanguages.includes(Language.GENERATED_FORMAL) &&
