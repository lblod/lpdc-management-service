...
export class ConceptSnapshotToConceptMergerDomainService {
  ...
  constructor(
    ...
    this._ensureLinkedAuthoritiesExistsAsCodeListDomainService =
  ...
  async merge(conceptSnapshotId: Iri) {
    const newConceptSnapshot =
      await this._conceptSnapshotRepository.findById(conceptSnapshotId);
    const conceptId = newConceptSnapshot.isVersionOf;
    const conceptExists = await this._conceptRepository.exists(conceptId);
    const concept: Concept | undefined = conceptExists
      ? await this._conceptRepository.findById(conceptId)
      : undefined;

    const newConceptSnapshotAlreadyLinkedToConcept =
      concept?.appliedConceptSnapshots
        .map((iri) => iri.value)
        .includes(newConceptSnapshot.id.value);

    if (newConceptSnapshotAlreadyLinkedToConcept) {
      this._logger.log(
        `The versioned resource <${conceptSnapshotId}> is already processed on service <${conceptId}>`,
      );

      await this._ensureLinkedAuthoritiesExistsAsCodeListDomainService.ensureLinkedAuthoritiesExistAsCodeList(
        [
          ...newConceptSnapshot.competentAuthorities,
          ...newConceptSnapshot.executingAuthorities,
        ],
      );
    ...
    if (conceptExists && !isNewerSnapshotThanAllPreviouslyApplied) {
      ...
      const updatedConcept = this.addAsPreviousConceptSnapshot(
        newConceptSnapshot,
        concept,
      );
    ...
    this._logger.log(
      `New versioned resource found: ${conceptSnapshotId} of service ${conceptId}`,
    );

    const currentConceptSnapshotId: Iri | undefined =
      concept?.latestConceptSnapshot;
    const isConceptSnapshotFunctionallyChanged = await this.isConceptChanged(
      newConceptSnapshot,
      currentConceptSnapshotId,
    );
    ...
    if (!conceptExists) {
      ...
      const updatedConcept = this.asMergedConcept(
        newConceptSnapshot,
        concept,
        isConceptSnapshotFunctionallyChanged,
      );
      await this._conceptRepository.update(updatedConcept, concept);
    }

    await this._instanceRepository.updateReviewStatusesForInstances(
      conceptId,
      isConceptSnapshotFunctionallyChanged,
      newConceptSnapshot.isArchived,
    );
    ...
    await this._conceptDisplayConfigurationRepository.ensureConceptDisplayConfigurationsForAllBestuurseenheden(
      conceptId,
    );
  }
  ...
  private asMergedConcept(
    conceptSnapshot: ConceptSnapshot,
    concept: Concept,
    isConceptSnapshotFunctionallyChanged: boolean,
  ): Concept {
    return new Concept(
      concept.id,
      concept.uuid,
  ...
  private addAsPreviousConceptSnapshot(
    conceptSnapshot: ConceptSnapshot,
    concept: Concept,
  ): Concept {
    return new Concept(
      ...
      concept.latestConceptSnapshot,
      uniqWith(
        [...concept.previousConceptSnapshots, conceptSnapshot.id],
        isEqual,
      ),
      concept.latestConceptSnapshot,
      concept.conceptTags,
  ...
  private async isConceptChanged(
    ...
    const currentConceptSnapshot =
      await this._conceptSnapshotRepository.findById(currentSnapshotId);
