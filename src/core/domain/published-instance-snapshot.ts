...
export class PublishedInstanceSnapshot {
  ...
  constructor(
    ...
    this._isPublishedVersionOf = requiredValue(
    ...
    requireAtLeastOneValuePresentIfCondition(
      this._yourEuropeCategories,
      "yourEuropeCategories",
      () => publicationMedia.includes(PublicationMediumType.YOUREUROPE),
    );
    ...
    this._requirements = [...requirements].map((r) =>
      Requirement.forInstanceSnapshot(r),
    );
    requireNoDuplicates(
      this._requirements.map((r) => r.order),
      "requirements > order",
    );
    this._procedures = [...procedures].map((p) =>
      Procedure.forInstanceSnapshot(p),
    );
    requireNoDuplicates(
      this._procedures.map((r) => r.order),
      "procedures > order",
    );
    ...
    this._financialAdvantages = [...financialAdvantages].map((fa) =>
      FinancialAdvantage.forInstanceSnapshot(fa),
    );
    requireNoDuplicates(
    ...
    this._contactPoints = [...contactPoints].map((cp) =>
      ContactPoint.forInstanceSnapshot(cp),
    );
    requireNoDuplicates(
    ...
    this._legalResources = [...legalResources].map(
      LegalResource.forInstanceSnapshot,
    );
    requireNoDuplicates(
...
export class PublishedInstanceSnapshotBuilder {
  static buildIri(uniqueId: string): Iri {
    return new Iri(
      `http://data.lblod.info/id/published-public-service/${uniqueId}`,
    );
  }
  ...
  public static from(instance: Instance): PublishedInstanceSnapshot {
    ...
    return new PublishedInstanceSnapshot(
      ...
      instance.keywords,
      instance.requirements.map((req) =>
        Requirement.forInstanceSnapshot(req.transformWithNewId()),
      ),
      instance.procedures.map((proc) =>
        Procedure.forInstanceSnapshot(proc.transformWithNewId()),
      ),
      instance.websites.map((ws) =>
        Website.forInstanceSnapshot(ws.transformWithNewId()),
      ),
      instance.costs.map((c) =>
        Cost.forInstanceSnapshot(c.transformWithNewId()),
      ),
      instance.financialAdvantages.map((fa) =>
        FinancialAdvantage.forInstanceSnapshot(fa.transformWithNewId()),
      ),
      instance.contactPoints.map((cp) =>
        ContactPoint.forInstanceSnapshot(cp.transformWithNewId()),
      ),
      instance.conceptId,
      instance.languages,
      ...
      instance.spatials,
      instance.legalResources.map((lr) =>
        LegalResource.forInstanceSnapshot(lr.transformWithNewId()),
      ),
    );
  }
}
