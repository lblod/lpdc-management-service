...
import {
  buildFormalInformalChoiceIri,
  FormalInformalChoice,
} from "./formal-informal-choice";
...
export class NewFormalInformalChoiceDomainService {
  ...
  public async saveFormalInformalChoiceAndSyncInstances(
    bestuurseenheid: Bestuurseenheid,
    chosenForm: ChosenFormType,
  ): Promise<FormalInformalChoice> {
    const formalInformalChoose = await this.saveFormalInformalChoice(
      bestuurseenheid,
      chosenForm,
    );
    await this._instanceRepository.syncNeedsConversionFromFormalToInformal(
      bestuurseenheid,
      formalInformalChoose.chosenForm,
    );

    return formalInformalChoose;
  }

  private async saveFormalInformalChoice(
    bestuurseenheid: Bestuurseenheid,
    chosenForm: ChosenFormType,
  ): Promise<FormalInformalChoice> {
    const newUuid = uuid();
    ...
    await this._formalInformalChoiceRepository.save(
      bestuurseenheid,
      formalInformalChoice,
    );
