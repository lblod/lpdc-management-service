...
function checkDependency(source: string, acceptedSources: string[]): string[] {
  ...
  const acceptedDirs = acceptedSources.map((source) =>
    ...
    if (sourceFiles.includes(sourceFile.fileName)) {
      ts.forEachChild(sourceFile, (node) => {
        if (ts.isImportDeclaration(node)) {
          ...
          if (
            fullImportPath.startsWith(projectRoot) &&
            !acceptedDirs.some((access) => fullImportPath.startsWith(access))
          ) {
            const relativeImportPath = relative(projectRoot, fullImportPath);
            const relativeSourceFilePath = relative(
              projectRoot,
              sourceFile.fileName,
            );
            violations.push(
              `Illegal import: '${relativeImportPath}' in '${relativeSourceFilePath}'`,
            );
          }
        }
      });
...
function getAllFilesAndNestedFilesFromDir(
  dir: string,
  filelist: string[] = [],
): string[] {
