...
describe("InstanceSnapshotProcessorApplicationService", () => {
  ...
  const linkedAuthoritiesDomainService =
  ...
  const instanceSnapshotProcessor =
    new InstanceSnapshotProcessorApplicationService(
      instanceSnapshotMerger,
  ...
  describe("retry merge", () => {
    ...
    beforeEach(() => {
      spy = spyOn(instanceSnapshotMerger, "merge").mockRejectedValue({
        message: "Some error",
      });
    });
