...
describe("Update Instance Application Service tests", () => {
  ...
  test("Can update an instance by setting an initial title, and updating that title in a second update using a turtle input", async () => {
    ...
    let updatedInstance = await instanceRepository.findById(
      bestuurseenheid,
      instance.id,
    );
...
pub:${instance.uuid} dct:title "initial title"@nl-be-x-formal.\n\n`,
    ...
    updatedInstance = await instanceRepository.findById(
      bestuurseenheid,
      instance.id,
    );
      ...
      InstanceBuilder.from(instance)
        .withProcedures([
          new ProcedureBuilder()
            .withId(
              new Iri(
                "http://data.lblod.info/form-data/nodes/02c296ca-d194-4971-9325-e17809afe087",
              ),
            )
            ...
            .withDescription(
              LanguageString.of(
                undefined,
                `<p data-indentation-level="0">beschrijving procedure</p>`,
              ),
            )
            .withWebsites([])
            .withOrder(1)
