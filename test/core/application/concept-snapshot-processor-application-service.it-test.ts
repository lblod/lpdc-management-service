...
describe("ConceptSnapshotProcessorApplicationService", () => {
  ...
  const instanceSnapshotRepository = new InstanceSparqlRepository(
    TEST_SPARQL_ENDPOINT,
  );
  ...
  const conceptSnapshotProcessor =
  ...
  describe("retry merge", () => {
    ...
    beforeEach(() => {
      spy = spyOn(conceptSnapshotMerger, "merge").mockRejectedValue({
        message: "Some error",
      });
    });
  ...
  test("merge conceptSnapshots with same concept in correct order", async () => {
    ...
    expect(actual.appliedConceptSnapshots).toEqual(
      expect.arrayContaining([conceptSnapshot1.id, conceptSnapshot2.id]),
    );
  });
});
