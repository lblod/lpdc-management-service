...
describe("merges a new concept snapshot into a concept", () => {
  ...
  const conceptSnapshotRepository = new ConceptSnapshotSparqlTestRepository(
  ...
  describe("create a new concept", () => {
    test("Creates a new concept from a concept snapshot", async () => {
      ...
      await insertAllConceptSchemeLinksToGoOverGraphBoundaryVerifyConceptSchemesOfEnums(
        conceptSnapshot,
      );
      ...
      const createdConcept =
        await conceptRepository.findById(isVersionOfConceptId);
      ...
      expect(createdConcept.additionalDescription).toEqual(
        ...
        expect.arrayContaining([
          expect.objectContaining({
            _id: expect.not.objectContaining(
              conceptSnapshot.requirements[0].id,
            ),
            ...
            _evidence: expect.objectContaining({
              _id: expect.not.objectContaining(
                conceptSnapshot.requirements[0].evidence.id,
              ),
              _uuid: expect.stringMatching(uuidRegex),
              _title: conceptSnapshot.requirements[0].evidence.title,
              _description:
                conceptSnapshot.requirements[0].evidence.description,
            }),
          }),
          expect.objectContaining({
            _id: expect.not.objectContaining(
              conceptSnapshot.requirements[1].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
            ...
            _evidence: expect.objectContaining({
              _id: expect.not.objectContaining(
                conceptSnapshot.requirements[1].evidence.id,
              ),
              _uuid: expect.stringMatching(uuidRegex),
              _title: conceptSnapshot.requirements[1].evidence.title,
              _description:
                conceptSnapshot.requirements[1].evidence.description,
            }),
          }),
        ]),
      );
      ...
      expect(createdConcept.financialAdvantages).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            _id: expect.not.objectContaining(
              conceptSnapshot.financialAdvantages[0].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
          ...
          expect.objectContaining({
            _id: expect.not.objectContaining(
              conceptSnapshot.financialAdvantages[1].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
      ...
      expect(createdConcept.latestFunctionallyChangedConceptSnapshot).toEqual(
        conceptSnapshot.id,
      );
    ...
    test("Creates a new concept from a concept snapshot with keywords with other nl languages than nl ", async () => {
      ...
      insertAllConceptSchemeLinksToGoOverGraphBoundaryVerifyConceptSchemesOfEnums(
        conceptSnapshot,
      );
  ...
  describe("updates a concept", () => {
    test("Updates a concept with all new data of a new version of a concept snapshot", async () => {
      ...
      const conceptSnapshot = aFullConceptSnapshot()
        ...
        .withTargetAudiences([
          ...
          TargetAudienceType.ORGANISATIE,
        ])
        .withThemes([
          ThemeType.CULTUURSPORTVRIJETIJD,
          ThemeType.MOBILITEITOPENBAREWERKEN,
        ...
        .withYourEuropeCategories([
          YourEuropeCategoryType.BEDRIJFINSOLVENTIELIQUIDATIE,
          YourEuropeCategoryType.PROCEDUREPENSIONERING,
          YourEuropeCategoryType.GOEDERENRECYCLAGE,
        ])
        .withKeywords([
          LanguageString.of("buitenland"),
          LanguageString.of("ambulante activiteit"),
          LanguageString.of("levensloos"),
        ])
        .withGeneratedAtTime(FormatPreservingDate.of("2023-12-10T00:00:00Z"))
        .withConceptTags([ConceptTagType.YOUREUROPEVERPLICHT])
        .withLegalResources([
          anotherFullLegalResourceForConceptSnapshot(uuid())
            .withOrder(1)
            .build(),
          anotherFullLegalResourceForConceptSnapshot(uuid())
            .withOrder(2)
      ...
      const updatedConceptSnapshot = aMinimalConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
        ...
        .withAdditionalDescription(
          suffixUnique(conceptSnapshot.additionalDescription),
        )
        ...
        .withTargetAudiences([
          TargetAudienceType.ONDERNEMING,
          TargetAudienceType.VERENIGING,
        ])
        .withThemes([
          ThemeType.BOUWENWONEN,
          ThemeType.ONDERWIJSWETENSCHAP,
          ThemeType.MILIEUENERGIE,
        ])
        .withCompetentAuthorityLevels([
        ...
        .withYourEuropeCategories([
          YourEuropeCategoryType.CONSUMENTENRECHTEN,
          YourEuropeCategoryType.PROCEDURESTARTENEXPLOITERENSLUITENBEDRIJFKENNISGEVING,
          YourEuropeCategoryType.BURGERENFAMILIERECHTEN,
        ])
        .withKeywords([
          LanguageString.of("groenvoorziening"),
          LanguageString.of("green"),
          LanguageString.of("huis en tuin verwerking"),
        ])
        .withRequirements([
          anotherFullRequirement()
            .withTitle(suffixUnique(conceptSnapshot.requirements[1].title))
            .withDescription(
              suffixUnique(conceptSnapshot.requirements[1].description),
            )
            .withOrder(1)
            .withEvidence(
              aFullEvidence()
                .withTitle(
                  suffixUnique(conceptSnapshot.requirements[1].evidence.title),
                )
                .withDescription(
                  suffixUnique(
                    conceptSnapshot.requirements[1].evidence.description,
          ...
          aFullRequirement()
            ...
            .withDescription(
              suffixUnique(conceptSnapshot.requirements[0].description),
            )
            .withOrder(2)
            .withEvidence(
              anotherFullEvidence()
                .withTitle(
                  suffixUnique(conceptSnapshot.requirements[0].evidence.title),
                )
                .withDescription(
                  suffixUnique(
                    conceptSnapshot.requirements[0].evidence.description,
        ...
        .withProcedures([
          anotherFullProcedure()
            ...
            .withDescription(
              suffixUnique(conceptSnapshot.procedures[1].description),
            )
            .withOrder(1)
            .withWebsites([
              anotherFullWebsite(uuid()).withOrder(1).build(),
              anotherFullWebsite(uuid()).withOrder(2).build(),
              anotherFullWebsite(uuid()).withOrder(3).build(),
            ])
            .build(),
          aFullProcedure()
            .withTitle(suffixUnique(conceptSnapshot.procedures[0].title))
            .withDescription(
              suffixUnique(conceptSnapshot.procedures[0].description),
            )
            .withOrder(2)
            .withWebsites([
              anotherFullWebsite(uuid()).withOrder(1).build(),
        ...
        .withFinancialAdvantages([
          anotherFullFinancialAdvantage()
            .withTitle(
              suffixUnique(conceptSnapshot.financialAdvantages[1].title),
            )
            .withDescription(
              suffixUnique(conceptSnapshot.financialAdvantages[1].description),
            )
            .withOrder(1)
            .build(),
          aFullFinancialAdvantage()
            .withTitle(
              suffixUnique(conceptSnapshot.financialAdvantages[0].title),
            )
            .withDescription(
              suffixUnique(conceptSnapshot.financialAdvantages[0].description),
            )
            .withOrder(2)
            .build(),
        ])
      ...
      expect(updatedConcept.description).toEqual(
        updatedConceptSnapshot.description,
      );
      expect(updatedConcept.additionalDescription).toEqual(
        updatedConceptSnapshot.additionalDescription,
      );
      expect(updatedConcept.exception).toEqual(
        updatedConceptSnapshot.exception,
      );
      expect(updatedConcept.regulation).toEqual(
        updatedConceptSnapshot.regulation,
      );
      expect(updatedConcept.startDate).toEqual(
        updatedConceptSnapshot.startDate,
      );
      ...
      expect(updatedConcept.requirements).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.requirements[0].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
            ...
            _evidence: expect.objectContaining({
              _id: expect.not.objectContaining(
                updatedConceptSnapshot.requirements[0].evidence.id,
              ),
              _uuid: expect.stringMatching(uuidRegex),
              _title: updatedConceptSnapshot.requirements[0].evidence.title,
              _description:
                updatedConceptSnapshot.requirements[0].evidence.description,
            }),
          }),
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.requirements[1].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
            ...
            _evidence: expect.objectContaining({
              _id: expect.not.objectContaining(
                updatedConceptSnapshot.requirements[1].evidence.id,
              ),
              _uuid: expect.stringMatching(uuidRegex),
              _title: updatedConceptSnapshot.requirements[1].evidence.title,
              _description:
                updatedConceptSnapshot.requirements[1].evidence.description,
            }),
          }),
        ]),
      );
      ...
      expect(updatedConcept.procedures).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.procedures[0].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
            ...
            _websites: expect.arrayContaining([
              expect.objectContaining({
                _id: expect.not.objectContaining(
                  updatedConceptSnapshot.procedures[0].websites[0].id,
                ),
                _uuid: expect.stringMatching(uuidRegex),
                _title: updatedConceptSnapshot.procedures[0].websites[0].title,
                _description:
                  updatedConceptSnapshot.procedures[0].websites[0].description,
                _order: 1,
                _url: updatedConceptSnapshot.procedures[0].websites[0].url,
              }),
              expect.objectContaining({
                _id: expect.not.objectContaining(
                  updatedConceptSnapshot.procedures[0].websites[1].id,
                ),
                _uuid: expect.stringMatching(uuidRegex),
                _title: updatedConceptSnapshot.procedures[0].websites[1].title,
                _description:
                  updatedConceptSnapshot.procedures[0].websites[1].description,
                _order: 2,
                _url: updatedConceptSnapshot.procedures[0].websites[1].url,
              }),
              expect.objectContaining({
                _id: expect.not.objectContaining(
                  updatedConceptSnapshot.procedures[0].websites[2].id,
                ),
                _uuid: expect.stringMatching(uuidRegex),
                _title: updatedConceptSnapshot.procedures[0].websites[2].title,
                _description:
                  updatedConceptSnapshot.procedures[0].websites[2].description,
                _order: 3,
          ...
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.procedures[1].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
            ...
            _websites: expect.arrayContaining([
              expect.objectContaining({
                _id: expect.not.objectContaining(
                  updatedConceptSnapshot.procedures[1].websites[0].id,
                ),
                _uuid: expect.stringMatching(uuidRegex),
                _title: updatedConceptSnapshot.procedures[1].websites[0].title,
                _description:
                  updatedConceptSnapshot.procedures[1].websites[0].description,
                _order: 1,
                _url: updatedConceptSnapshot.procedures[1].websites[0].url,
              }),
              expect.objectContaining({
                _id: expect.not.objectContaining(
                  updatedConceptSnapshot.procedures[1].websites[1].id,
                ),
                _uuid: expect.stringMatching(uuidRegex),
                _title: updatedConceptSnapshot.procedures[1].websites[1].title,
                _description:
                  updatedConceptSnapshot.procedures[1].websites[1].description,
                _order: 2,
      ...
      expect(updatedConcept.websites).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.websites[0].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
          ...
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.websites[1].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
      ...
      expect(updatedConcept.costs).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.costs[0].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
          ...
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.costs[1].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
      ...
      expect(updatedConcept.financialAdvantages).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.financialAdvantages[0].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
            _title: updatedConceptSnapshot.financialAdvantages[0].title,
            _description:
              updatedConceptSnapshot.financialAdvantages[0].description,
          }),
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.financialAdvantages[1].id,
            ),
            _uuid: expect.stringMatching(uuidRegex),
            _title: updatedConceptSnapshot.financialAdvantages[1].title,
            _description:
              updatedConceptSnapshot.financialAdvantages[1].description,
          }),
        ]),
      );

      expect(updatedConcept.productId).toEqual(
        updatedConceptSnapshot.productId,
      );
      expect(updatedConcept.latestConceptSnapshot).toEqual(
        updatedConceptSnapshot.id,
      );
      expect(updatedConcept.previousConceptSnapshots.sort()).toEqual(
        [conceptSnapshot.id].sort(),
      );
      expect(updatedConcept.latestFunctionallyChangedConceptSnapshot).toEqual(
        updatedConceptSnapshot.id,
      );
      expect(updatedConcept.conceptTags).toEqual(
        updatedConceptSnapshot.conceptTags,
      );
      expect(updatedConcept.legalResources).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.legalResources[0],
            ),
            _uuid: expect.stringMatching(uuidRegex),
          ...
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.legalResources[1],
            ),
            _uuid: expect.stringMatching(uuidRegex),
          ...
          expect.objectContaining({
            _id: expect.not.objectContaining(
              updatedConceptSnapshot.legalResources[2],
            ),
            _uuid: expect.stringMatching(uuidRegex),
    ...
    test("Only updates the previousConceptSnapshot data of the concept when newer version already processed", async () => {
      ...
      const notUpdatedConcept =
        await conceptRepository.findById(isVersionOfConceptId);
      expect(notUpdatedConcept.title).toEqual(conceptSnapshot.title);
      expect(notUpdatedConcept.latestConceptSnapshot).toEqual(
        conceptSnapshot.id,
      );
      expect(notUpdatedConcept.previousConceptSnapshots.sort()).toEqual(
        [updatedConceptSnapshot.id].sort(),
      );
      expect(
        notUpdatedConcept.latestFunctionallyChangedConceptSnapshot,
      ).toEqual(conceptSnapshot.id);

      const anotherOldUpdatedConceptSnapshot = aMinimalConceptSnapshot()
        .withTitle(suffixUnique(conceptSnapshot.title))
        .withIsVersionOfConcept(isVersionOfConceptId)
        .withGeneratedAtTime(FormatPreservingDate.of("2023-12-10T00:00:00Z"))
        .build();
      ...
      await insertAllConceptSchemeLinksToGoOverGraphBoundaryVerifyConceptSchemesOfEnums(
        anotherOldUpdatedConceptSnapshot,
      );
      ...
      const againNotUpdatedConcept =
        await conceptRepository.findById(isVersionOfConceptId);
      expect(againNotUpdatedConcept.title).toEqual(conceptSnapshot.title);
      expect(againNotUpdatedConcept.latestConceptSnapshot).toEqual(
        conceptSnapshot.id,
      );
      expect(againNotUpdatedConcept.previousConceptSnapshots.sort()).toEqual(
        [anotherOldUpdatedConceptSnapshot.id, updatedConceptSnapshot.id].sort(),
      );
      expect(
        againNotUpdatedConcept.latestFunctionallyChangedConceptSnapshot,
      ).toEqual(conceptSnapshot.id);
    }, 10000);
    ...
    test("Updates a concept from a concept snapshot that is archived and then unarchived", async () => {
      ...
      const archivedConceptSnapshot = aMinimalConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
        .withIsArchived(true)
        .withCosts([aFullCost().build()])
        .withGeneratedAtTime(FormatPreservingDate.of("2023-12-11T00:00:00Z"))
      ...
      const unarchivedConceptSnapshot = aMinimalConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
    ...
    test("Multiple consecutive updates to a concept", async () => {
      ...
      const secondTimeUpdatedConceptSnapshot = aMinimalConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
      ...
      await insertAllConceptSchemeLinksToGoOverGraphBoundaryVerifyConceptSchemesOfEnums(
        secondTimeUpdatedConceptSnapshot,
      );
      ...
      const secondTimeUpdatedConcept =
        await conceptRepository.findById(isVersionOfConceptId);
      expect(secondTimeUpdatedConcept.id).toEqual(isVersionOfConceptId);
      expect(secondTimeUpdatedConcept.uuid).toMatch(uuidRegex);
      expect(secondTimeUpdatedConcept.title).toEqual(
        secondTimeUpdatedConceptSnapshot.title,
      );
      expect(secondTimeUpdatedConcept.latestConceptSnapshot).toEqual(
        secondTimeUpdatedConceptSnapshot.id,
      );
      expect(secondTimeUpdatedConcept.previousConceptSnapshots.sort()).toEqual(
        [conceptSnapshot.id, updatedConceptSnapshot.id].sort(),
      );
      expect(
        secondTimeUpdatedConcept.latestFunctionallyChangedConceptSnapshot,
      ).toEqual(secondTimeUpdatedConceptSnapshot.id);

      const thirdTimeButOlderUpdatedConceptSnapshot = aMinimalConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
        .withTitle(suffixUnique(updatedConceptSnapshot.title))
      ...
      await insertAllConceptSchemeLinksToGoOverGraphBoundaryVerifyConceptSchemesOfEnums(
        thirdTimeButOlderUpdatedConceptSnapshot,
      );

      await conceptSnapshotRepository.save(
        thirdTimeButOlderUpdatedConceptSnapshot,
      );

      await merger.merge(thirdTimeButOlderUpdatedConceptSnapshot.id);

      const notUpdatedForThirdTimeUpdatedConcept =
        await conceptRepository.findById(isVersionOfConceptId);
      expect(notUpdatedForThirdTimeUpdatedConcept.id).toEqual(
        isVersionOfConceptId,
      );
      expect(notUpdatedForThirdTimeUpdatedConcept.uuid).toMatch(uuidRegex);
      expect(notUpdatedForThirdTimeUpdatedConcept.title).toEqual(
        secondTimeUpdatedConceptSnapshot.title,
      );
      expect(
        notUpdatedForThirdTimeUpdatedConcept.latestConceptSnapshot,
      ).toEqual(secondTimeUpdatedConceptSnapshot.id);
      expect(
        notUpdatedForThirdTimeUpdatedConcept.previousConceptSnapshots.sort(),
      ).toEqual(
        [
          conceptSnapshot.id,
          updatedConceptSnapshot.id,
          thirdTimeButOlderUpdatedConceptSnapshot.id,
        ].sort(),
      );
      expect(
        notUpdatedForThirdTimeUpdatedConcept.latestFunctionallyChangedConceptSnapshot,
      ).toEqual(secondTimeUpdatedConceptSnapshot.id);

      const fourthTimeUpdatedConceptSnapshot = aMinimalConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
      ...
      await insertAllConceptSchemeLinksToGoOverGraphBoundaryVerifyConceptSchemesOfEnums(
        fourthTimeUpdatedConceptSnapshot,
      );
      ...
      const fourthTimeUpdatedConcept =
        await conceptRepository.findById(isVersionOfConceptId);
      expect(fourthTimeUpdatedConcept.id).toEqual(isVersionOfConceptId);
      expect(fourthTimeUpdatedConcept.uuid).toMatch(uuidRegex);
      expect(fourthTimeUpdatedConcept.title).toEqual(
        fourthTimeUpdatedConceptSnapshot.title,
      );
      expect(fourthTimeUpdatedConcept.latestConceptSnapshot).toEqual(
        fourthTimeUpdatedConceptSnapshot.id,
      );
      expect(fourthTimeUpdatedConcept.previousConceptSnapshots.sort()).toEqual(
        [
          conceptSnapshot.id,
          updatedConceptSnapshot.id,
          secondTimeUpdatedConceptSnapshot.id,
      ...
      expect(
        fourthTimeUpdatedConcept.latestFunctionallyChangedConceptSnapshot,
      ).toEqual(fourthTimeUpdatedConceptSnapshot.id);
    }, 30000);
  ...
  test("Creates a concept display configuration for each bestuurseenheid", async () => {
    ...
    const createdConceptDisplayConfigurationForBestuurseenheid =
      await conceptDisplayConfigurationRepository.findByConceptId(
        bestuurseenheid,
        isVersionOfConceptId,
      );
    expect(
      createdConceptDisplayConfigurationForBestuurseenheid.id,
    ).not.toBeUndefined();
    expect(
      createdConceptDisplayConfigurationForBestuurseenheid.uuid,
    ).not.toBeUndefined();
    expect(
      createdConceptDisplayConfigurationForBestuurseenheid.conceptIsNew,
    ).toEqual(true);
    expect(
      createdConceptDisplayConfigurationForBestuurseenheid.conceptIsInstantiated,
    ).toEqual(false);
    expect(
      createdConceptDisplayConfigurationForBestuurseenheid.bestuurseenheidId,
    ).toEqual(bestuurseenheid.id);
    expect(
      createdConceptDisplayConfigurationForBestuurseenheid.conceptId,
    ).toEqual(isVersionOfConceptId);

    const createdConceptDisplayConfigurationForAnotherBestuurseenheid =
      await conceptDisplayConfigurationRepository.findByConceptId(
        anotherBestuurseenheid,
        isVersionOfConceptId,
      );
    expect(
      createdConceptDisplayConfigurationForAnotherBestuurseenheid.id,
    ).not.toBeUndefined();
    expect(
      createdConceptDisplayConfigurationForAnotherBestuurseenheid.uuid,
    ).not.toBeUndefined();
    expect(
      createdConceptDisplayConfigurationForAnotherBestuurseenheid.conceptIsNew,
    ).toEqual(true);
    expect(
      createdConceptDisplayConfigurationForAnotherBestuurseenheid.conceptIsInstantiated,
    ).toEqual(false);
    expect(
      createdConceptDisplayConfigurationForAnotherBestuurseenheid.bestuurseenheidId,
    ).toEqual(anotherBestuurseenheid.id);
    expect(
      createdConceptDisplayConfigurationForAnotherBestuurseenheid.conceptId,
    ).toEqual(isVersionOfConceptId);
  }, 10000);
  ...
  describe("Update instance review statuses", () => {
    test("Updates instance review status to updated for each linked instance if concept is FunctionallyModified", async () => {
      ...
      await directDatabaseAccess.insertData(
        ...
        [PREFIX.lpdcExt, PREFIX.dct, PREFIX.schema],
      );
      ...
      const reviewStatusAndDateModifiedForConceptInGraph = (
        be: Bestuurseenheid,
      ) => `           
                ${PREFIX.lpdcExt}
                ${PREFIX.dct}
            ...
            `;
      const reviewStatusAndDateModifiedResultForInstanceOfBestuurseenheid =
        await directDatabaseAccess.list(
          reviewStatusAndDateModifiedForConceptInGraph(bestuurseenheid),
        );
      expect(
        reviewStatusAndDateModifiedResultForInstanceOfBestuurseenheid.length,
      ).toEqual(1);

      const reviewStatusForInstanceOfBestuurseenheid =
        reviewStatusAndDateModifiedResultForInstanceOfBestuurseenheid[0][
          "reviewStatus"
        ].value;
      expect(reviewStatusForInstanceOfBestuurseenheid).toEqual(
        NS.concepts.reviewStatus(InstanceReviewStatusType.CONCEPT_GEWIJZIGD)
          .value,
      );

      const dateModifiedForInstanceOfBestuurseenheid =
        reviewStatusAndDateModifiedResultForInstanceOfBestuurseenheid[0][
          "dateModified"
        ].value;
      expect(dateModifiedForInstanceOfBestuurseenheid).toEqual(
        previousDateModified.toISOString(),
      );

      const reviewStatusResultForInstanceOfAnotherBestuurseenheid =
        await directDatabaseAccess.list(
          reviewStatusAndDateModifiedForConceptInGraph(anotherBestuurseenheid),
        );
      expect(
        reviewStatusResultForInstanceOfAnotherBestuurseenheid.length,
      ).toEqual(1);

      const reviewStatusForInstanceOfAnotherBestuurseenheid =
        reviewStatusResultForInstanceOfAnotherBestuurseenheid[0]["reviewStatus"]
          .value;
      expect(reviewStatusForInstanceOfAnotherBestuurseenheid).toEqual(
        NS.concepts.reviewStatus(InstanceReviewStatusType.CONCEPT_GEWIJZIGD)
          .value,
      );

      const dateModifiedForInstanceOfAnotherBestuurseenheid =
        reviewStatusResultForInstanceOfAnotherBestuurseenheid[0]["dateModified"]
          .value;
      expect(dateModifiedForInstanceOfAnotherBestuurseenheid).toEqual(
        previousDateModified.toISOString(),
      );
    }, 20000);
    ...
    test("Does not updates instance review status to updated for each linked instance if concept is not FunctionallyModified", async () => {
      ...
      await directDatabaseAccess.insertData(
        ...
        [PREFIX.lpdcExt, PREFIX.dct],
      );

      const updatedConceptSnapshot = aFullConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
        .withGeneratedAtTime(FormatPreservingDate.of("2023-12-11T00:00:00Z"))
        .build();
      ...
      const reviewStatusForConceptInGraph = (
        be: Bestuurseenheid,
      ) => `           
                ${PREFIX.lpdcExt}
                ${PREFIX.dct}
            ...
            `;
      const reviewStatusResultForInstance = await directDatabaseAccess.list(
        reviewStatusForConceptInGraph(bestuurseenheid),
      );
    ...
    test("Updates instance review status to archived for each linked instance if concept is archived", async () => {
      ...
      const reviewStatusAndDateModifiedForConceptInGraph = (
        be: Bestuurseenheid,
            ...
            `;
      const reviewStatusAndDateModifiedResultForInstance =
        await directDatabaseAccess.list(
          reviewStatusAndDateModifiedForConceptInGraph(bestuurseenheid),
        );
      expect(reviewStatusAndDateModifiedResultForInstance.length).toEqual(1);

      const reviewStatusForInstance =
        reviewStatusAndDateModifiedResultForInstance[0]["reviewStatus"].value;
      expect(reviewStatusForInstance).toEqual(
        NS.concepts.reviewStatus(InstanceReviewStatusType.CONCEPT_GEARCHIVEERD)
          .value,
      );

      const dateModifiedForInstance =
        reviewStatusAndDateModifiedResultForInstance[0]["dateModified"].value;
      expect(dateModifiedForInstance).toEqual(
        previousDateModified.toISOString(),
      );
    });
    ...
    test("Updates instance review status to Concept gewijzigd for each linked instance if concept is archived and then unarchived", async () => {
      ...
      const archivedConceptSnapshot = aFullConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
        .withIsArchived(true)
        .withGeneratedAtTime(FormatPreservingDate.of("2023-12-11T00:00:00Z"))
        .build();
      ...
      const reviewStatusForInstance =
        reviewStatusResultForInstance[0]["reviewStatus"].value;
      expect(reviewStatusForInstance).toEqual(
        NS.concepts.reviewStatus(InstanceReviewStatusType.CONCEPT_GEARCHIVEERD)
          .value,
      );

      const unarchivedConceptSnapshot = aFullConceptSnapshot()
        .withIsVersionOfConcept(isVersionOfConceptId)
        .withIsArchived(false)
        .withGeneratedAtTime(FormatPreservingDate.of("2023-12-12T00:00:00Z"))
        .build();
      ...
      const reviewStatusAndDateModifiedForConceptInGraphAfterUnarchive = (
        be: Bestuurseenheid,
      ) => `           
                ${PREFIX.lpdcExt}
                ${PREFIX.dct}
            ...
            `;
      const reviewStatusAndDateModifiedResultForInstanceAfterUnarchive =
        await directDatabaseAccess.list(
          reviewStatusAndDateModifiedForConceptInGraphAfterUnarchive(
            bestuurseenheid,
          ),
        );
      expect(
        reviewStatusAndDateModifiedResultForInstanceAfterUnarchive.length,
      ).toEqual(1);

      const reviewStatusForInstanceAfterUnarchive =
        reviewStatusAndDateModifiedResultForInstanceAfterUnarchive[0][
          "reviewStatus"
        ].value;
      expect(reviewStatusForInstanceAfterUnarchive).toEqual(
        NS.concepts.reviewStatus(InstanceReviewStatusType.CONCEPT_GEWIJZIGD)
          .value,
      );

      const dateModifiedForInstanceAfterUnarchive =
        reviewStatusAndDateModifiedResultForInstanceAfterUnarchive[0][
          "dateModified"
        ].value;
      expect(dateModifiedForInstanceAfterUnarchive).toEqual(
        previousDateModified.toISOString(),
      );
    });
  });
  ...
  async function insertAllConceptSchemeLinksToGoOverGraphBoundaryVerifyConceptSchemesOfEnums(
    conceptSnapshot: ConceptSnapshot,
  ) {
    const triples = [
      conceptSnapshot.type
        ? `<${NS.dvc.type(conceptSnapshot.type).value}> skos:inScheme <${NS.dvcs("Type").value}>`
        : undefined,
      conceptSnapshot.type
        ? `<${NS.dvc.type(conceptSnapshot.type).value}> a skos:Concept`
        : undefined,
      ...conceptSnapshot.targetAudiences.flatMap((v) => [
        `<${NS.dvc.doelgroep(v).value}> skos:inScheme <${NS.dvcs("Doelgroep").value}>`,
        `<${NS.dvc.doelgroep(v).value}> a skos:Concept`,
      ]),
      ...conceptSnapshot.themes.flatMap((v) => [
        `<${NS.dvc.thema(v).value}> skos:inScheme <${NS.dvcs("Thema").value}>`,
        `<${NS.dvc.thema(v).value}> a skos:Concept`,
      ]),
      ...conceptSnapshot.competentAuthorityLevels.flatMap((v) => [
      ...
      ...conceptSnapshot.executingAuthorityLevels.flatMap((v) => [
      ...
      ...conceptSnapshot.publicationMedia.flatMap((v) => [
      ...
      ...conceptSnapshot.yourEuropeCategories.flatMap((v) => [
