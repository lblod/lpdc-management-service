  ...
  describe("ConfirmUpToDateTill", () => {
    test("should update conceptSnapshot on instance", async () => {
      ...
      const instance = aFullInstance()
        ...
        .build();
      const newConceptSnapshot = aFullConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
        .build();
      ...
      const actualInstance = await instanceRepository.findById(
        bestuurseenheid,
        instance.id,
      );
    ...
    test("when new conceptSnapshot is latestFunctionalChangedConceptSnapshot and reviewStatus is set of concept then reviewStatus should be undefined", async () => {
      const bestuurseenheid = aBestuurseenheid().build();
      const conceptId = buildConceptIri(uuid());
      const currentConceptSnapshot = aFullConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
        .build();
      const latestFunctionalChangedSnapshot = aFullConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
        .build();
      const concept = aFullConcept()
        .withId(conceptId)
        .withLatestFunctionallyChangedConceptSnapshot(
          latestFunctionalChangedSnapshot.id,
        )
        .build();
      const instance = aFullInstance()
        .withConceptId(concept.id)
    ...
    test("when new conceptSnapshot is older than latestFunctionalChangedConceptSnapshot of concept then reviewStatus should not be updated", async () => {
      ...
      const conceptSnapshot1 = aFullConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
        .withGeneratedAtTime(date1)
        .build();
      const conceptSnapshot2 = aFullConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
        .withGeneratedAtTime(date2)
        .build();
      const conceptSnapshot3 = aFullConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
        .withGeneratedAtTime(date3)
        .build();
      const conceptSnapshot4 = aFullConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
        .withGeneratedAtTime(date4)
        .build();
      const concept = aFullConcept()
        .withId(conceptId)
  ...
  describe("Fully Take Concept Snapshot Over", () => {
    test("should override instance fields with fields from concept snapshot, takes over formal when instance has dutch language variant formal", async () => {
      ...
      const newConceptSnapshot = aFullConceptSnapshot()
        ...
        .withKeywords([
          ...
          LanguageString.of(undefined, "not nl"),
        ])
        .withRequirements([anotherFullRequirement().withOrder(1).build()])
        .withProcedures([
          anotherFullProcedure()
            .withOrder(1)
            .withWebsites([anotherFullWebsite(uuid()).withOrder(1).build()])
            .build(),
        ])
        .withWebsites([
        ...
        .withFinancialAdvantages([
          anotherFullFinancialAdvantage().withOrder(1).build(),
        ])
        .withLegalResources([
      ...
      const expectedInstance = InstanceBuilder.from(instance)
        ...
        .withYourEuropeCategories(
          ConceptSnapshotTestBuilder.YOUR_EUROPE_CATEGORIES,
        )
        .withKeywords([
          LanguageString.of("buitenland"),
          LanguageString.of("levensloos"),
        ...
        .withRequirements([
          RequirementBuilder.from(newConceptSnapshot.requirements[0])
            ...
            .withTitle(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.requirements[0].title.nlFormal,
                Language.FORMAL,
              ),
            )
            .withDescription(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.requirements[0].description.nlFormal,
                Language.FORMAL,
              ),
            )
            .withEvidence(
              EvidenceBuilder.from(newConceptSnapshot.requirements[0].evidence)
                ...
                .withTitle(
                  LanguageString.ofValueInLanguage(
                    newConceptSnapshot.requirements[0].evidence.title.nlFormal,
                    Language.FORMAL,
                  ),
                )
                .withDescription(
                  LanguageString.ofValueInLanguage(
                    newConceptSnapshot.requirements[0].evidence.description
                      .nlFormal,
                    Language.FORMAL,
                  ),
                )
        ...
        .withProcedures([
          ProcedureBuilder.from(newConceptSnapshot.procedures[0])
            ...
            .withTitle(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.procedures[0].title.nlFormal,
                Language.FORMAL,
              ),
            )
            .withDescription(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.procedures[0].description.nlFormal,
                Language.FORMAL,
              ),
            )
            .withWebsites([
              WebsiteBuilder.from(newConceptSnapshot.procedures[0].websites[0])
                ...
                .withTitle(
                  LanguageString.ofValueInLanguage(
                    newConceptSnapshot.procedures[0].websites[0].title.nlFormal,
                    Language.FORMAL,
                  ),
                )
                .withDescription(
                  LanguageString.ofValueInLanguage(
                    newConceptSnapshot.procedures[0].websites[0].description
                      .nlFormal,
                    Language.FORMAL,
                  ),
                )
        ...
        .withCosts([
          CostBuilder.from(newConceptSnapshot.costs[0])
            ...
            .withTitle(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.costs[0].title.nlFormal,
                Language.FORMAL,
              ),
            )
            .withDescription(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.costs[0].description.nlFormal,
                Language.FORMAL,
              ),
            )
            .build(),
        ...
        .withFinancialAdvantages([
          FinancialAdvantageBuilder.from(
            newConceptSnapshot.financialAdvantages[0],
          )
            .withId(expect.any(Object))
            .withUuid(expect.any(String))
            .withTitle(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.financialAdvantages[0].title.nlFormal,
                Language.FORMAL,
              ),
            )
            .withDescription(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.financialAdvantages[0].description.nlFormal,
                Language.FORMAL,
              ),
            )
            .build(),
        ...
        .withLegalResources([
          LegalResourceBuilder.from(newConceptSnapshot.legalResources[0])
            ...
            .withTitle(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.legalResources[0].title.nlFormal,
                Language.FORMAL,
              ),
            )
            .withDescription(
              LanguageString.ofValueInLanguage(
                newConceptSnapshot.legalResources[0].description.nlFormal,
                Language.FORMAL,
              ),
            )
            .build(),
    ...
    test("should override instance fields with fields from concept snapshot, takes over informal when instance has dutch language variant informal", async () => {
      ...
      const instance = aMinimalInstance()
        ...
        .withTitle(
          LanguageString.ofValueInLanguage(
            "title that needs updating",
            Language.INFORMAL,
          ),
        )
        .withProductId(InstanceTestBuilder.PRODUCT_ID)
        .build();
      const newConceptSnapshot = aMinimalConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
        .withTitle(
          LanguageString.of(
    ...
    test("when new conceptSnapshot is latestFunctionalChangedConceptSnapshot and reviewStatus is set of concept then reviewStatus should be undefined", async () => {
      ...
      expect(actualInstance.conceptSnapshotId).toEqual(
        latestFunctionalChangedSnapshot.id,
      );
      expect(actualInstance.reviewStatus).toBeUndefined();
      expect(actualInstance.dateModified).toEqual(FormatPreservingDate.now());
    ...
    test("when new conceptSnapshot is older than latestFunctionalChangedConceptSnapshot of concept then reviewStatus should not be updated", async () => {
      ...
      expect(actualInstance.reviewStatus).toEqual(
        InstanceReviewStatusType.CONCEPT_GEWIJZIGD,
      );
      expect(actualInstance.dateModified).toEqual(FormatPreservingDate.now());
    });
    ...
    test("when reviewStatus is different from concept gewijzigd throw error", async () => {
      ...
      const archivedConceptSnapshot = aFullConceptSnapshot()
        .withIsVersionOfConcept(archivedConceptId)
        .withIsArchived(true)
        .build();
      const newConceptSnapshot = aFullConceptSnapshot()
        .withIsVersionOfConcept(conceptId)
      ...
      await expect(
        ...
        InvariantError,
        "De review status is verschillend van concept-gewijzigd",
      );
      await expect(
        bringInstanceUpToDateWithConceptSnapshotVersionDomainService.fullyTakeConceptSnapshotOver(
          bestuurseenheid,
