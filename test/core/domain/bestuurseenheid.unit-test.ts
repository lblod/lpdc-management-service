...
test("spatials with duplicates throws error", () => {
  expect(() =>
    aBestuurseenheid()
      .withSpatials([buildNutsCodeIri(12312), buildNutsCodeIri(12312)])
      .build(),
  ).toThrow("spatials mag geen duplicaten bevatten");
});

test("userGraph", () => {
  const uniqueId = uuid();
  const bestuurseenheid = aBestuurseenheid().withUuid(uniqueId).build();

  expect(bestuurseenheid.userGraph().value).toEqual(
    `http://mu.semte.ch/graphs/organizations/${uniqueId}/LoketLB-LPDCGebruiker`,
  );
});
