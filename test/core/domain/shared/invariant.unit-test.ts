...
describe("notBeAbsent", () => {
  ...
  test("Returns error when value is undefined  ", () => {
    ...
    expect(() => invariant.to(invariant.notBeAbsent())).toThrowWithMessage(
      InvariantError,
      `${name} mag niet ontbreken`,
    );
  });
});
...
describe("notBeBlank", () => {
  ...
  test("Returns error when value is blank  ", () => {
    ...
    expect(() => invariant.to(invariant.notBeBlank())).toThrowWithMessage(
      InvariantError,
...
describe("haveAtLeastOneValuePresent", () => {
  ...
  test("Throws error when no value is present", () => {
    ...
    expect(() =>
      ...
      InvariantError,
      `${name} moet minstens een waarde bevatten`,
    );
  });
  ...
  test("Returns all values when one value is present", () => {
    ...
    expect(invariant.to(invariant.haveAtLeastOneValuePresent())).toEqual([
      "",
      undefined,
      "correct value",
    ]);
  ...
  test("Returns the multiple values when multiple values are present", () => {
    const invariant: Invariant<any> = Invariant.require(
      ["correct value 1", "", "correct value 2"],
      name,
    );
    expect(invariant.to(invariant.haveAtLeastOneValuePresent())).toEqual([
      "correct value 1",
...
describe("haveAtLeastXAmountOfValuesPresent", () => {
  ...
  test("Throws error when no fewer values are present", () => {
    ...
    expect(() =>
      invariant.to(invariant.haveAtLeastXAmountOfValues(expectedValues)),
    ).toThrowWithMessage(
      InvariantError,
      `${name} bevat minder dan ${expectedValues} waarden`,
    );
  });
...
describe("startsWith", () => {
  ...
  test("throws error when value is not of type string", () => {
    const invariant: Invariant<any> = Invariant.require([], name);
    expect(() => invariant.to(invariant.startsWith("a"))).toThrowWithMessage(
      InvariantError,
      `${name} begint niet met een van volgende waarden: [a]`,
    );
  });
  ...
  test("throws error when value does not start with", () => {
    const invariant: Invariant<any> = Invariant.require("abc", name);
    expect(() =>
      invariant.to(invariant.startsWith("b", "c")),
    ).toThrowWithMessage(
      InvariantError,
      `${name} begint niet met een van volgende waarden: [b,c]`,
    );
  });
});
...
describe("toMatchPattern", () => {
  ...
  test("throws error when value is not of type string", () => {
    const invariant: Invariant<any> = Invariant.require([], name);
    expect(() =>
      invariant.to(invariant.toMatchPattern(/\d{4}/)),
    ).toThrowWithMessage(
      InvariantError,
      `${name} komt niet overeen met het patroon`,
    );
  });
...
describe("noDuplicates", () => {
  ...
  test("Returns value when object list contains no duplicates", () => {
    ...
    expect(invariant.to(invariant.noDuplicates())).toEqual([
      { a: 0 },
      { a: 1 },
    ]);
  });
  ...
  test("throws error when string list contains duplicates", () => {
    ...
    expect(() => invariant.to(invariant.noDuplicates())).toThrowWithMessage(
      InvariantError,
      `${name} mag geen duplicaten bevatten`,
    );
  });
...
describe("allPresentOrAllAbsent", () => {
  ...
  test("Returns value when all elements are defined", () => {
    ...
    expect(invariant.to(invariant.allPresentOrAllAbsent())).toEqual([
      "a",
      "b",
      { a: 0 },
      { a: undefined },
  ...
  test("throws error when some elements are defined and others undefined", () => {
    ...
    expect(() =>
      invariant.to(invariant.allPresentOrAllAbsent()),
    ).toThrowWithMessage(
      InvariantError,
      `${name} moeten allemaal aanwezig of afwezig zijn`,
    );
  });
});
...
describe("canOnlyBePresentIfOtherValuePresent", () => {
  ...
  test("Returns value when presentValue and new value are undefined", () => {
    ...
    expect(
      invariant.to(
        invariant.canOnlyBePresentIfOtherValuePresent(undefined, presentName),
      ),
    ).toEqual(undefined);
  });
  ...
  test("Returns value when presentValue and new value are both present", () => {
    ...
    expect(
      invariant.to(
        invariant.canOnlyBePresentIfOtherValuePresent(
          presentValue,
          presentName,
        ),
      ),
  ...
  test("throws error when new value is defined and presentValue undefined", () => {
    ...
    expect(() =>
      ...
      InvariantError,
      `${name} kan alleen aanwezig zijn wanneer ${presentName} aanwezig is`,
    );
  });
});
...
describe("atLeastOneValuePresentIfCondition", () => {
  ...
  test("returns value when condition is met and at least on value is present", () => {
    ...
    expect(
      invariant.to(invariant.atLeastOneValuePresentIfCondition(() => true)),
    ).toEqual(["value"]);
  });
  ...
  test("returns value when condition is not met", () => {
    const invariant: Invariant<any> = Invariant.require(["value"], name);
    expect(
      invariant.to(invariant.atLeastOneValuePresentIfCondition(() => false)),
    ).toEqual(["value"]);
  });
...
describe("to", () => {
  test("returns first violation", () => {
    ...
    expect(() =>
      invariant.to(invariant.notBeBlank(), invariant.notBeAbsent()),
    ).toThrowWithMessage(InvariantError, `${name} mag niet leeg zijn`);
    expect(() =>
      invariant.to(invariant.notBeAbsent(), invariant.notBeBlank()),
    ).toThrowWithMessage(InvariantError, `${name} mag niet leeg zijn`);
  });
