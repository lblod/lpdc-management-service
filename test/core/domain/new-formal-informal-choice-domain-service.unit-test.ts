...
describe('new formal informal choice and sync needsConversionFromFormalInformalOnInstance domain service', () => {
    ...
    test('given formal, nl and informal instances, when choose informal, then saves formalInformalChoice and set needsConversionFromFormalToInformal on true for formal and nl instance', async () => {
        ...
        await instanceSparqlRepository.save(anotherBestuurseenheid, formalInstanceForOtherBestuurseenheid);
        await instanceSparqlRepository.save(anotherBestuurseenheid, informalInstanceForOtherBestuurseenheid);
        await instanceSparqlRepository.save(anotherBestuurseenheid, nlInstanceForOtherBestuurseenheid);

        const formalChoice = await newFormalInformalChoiceAndSyncInstanceDomainService.saveFormalInformalChoiceAndSyncInstances(bestuurseenheid, ChosenFormType.INFORMAL);

        const actualFormalInstance = await instanceSparqlRepository.findById(bestuurseenheid, formalInstance.id);
        const actualInformalInstance = await instanceSparqlRepository.findById(bestuurseenheid, informalInstance.id);
        const actualNlInstance = await instanceSparqlRepository.findById(bestuurseenheid, nlInstance.id);

        const actualFormalInstanceForOtherBestuurseenheid = await instanceSparqlRepository.findById(anotherBestuurseenheid, formalInstanceForOtherBestuurseenheid.id);
        const actualInformalInstanceForOtherBestuurseenheid = await instanceSparqlRepository.findById(anotherBestuurseenheid, informalInstanceForOtherBestuurseenheid.id);
        const actualNlInstanceForOtherBestuurseenheid = await instanceSparqlRepository.findById(anotherBestuurseenheid, nlInstanceForOtherBestuurseenheid.id);

        const actualFormalChoice = await formalInformalChoiceSparqlRepository.findByBestuurseenheid(bestuurseenheid);
    ...
    test('given formal, nl and informal instance, when choose formal, then saves formalInformalChoice and does not change needsConversionFromFormalToInformal', async () => {
        ...
        const formalChoice = await newFormalInformalChoiceAndSyncInstanceDomainService.saveFormalInformalChoiceAndSyncInstances(bestuurseenheid, ChosenFormType.FORMAL);
