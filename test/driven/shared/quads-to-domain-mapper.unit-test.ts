...
describe("quads to domain mapper", () => {
  ...
  describe("logs integrity problems", () => {
    ...
    test("No data integrity issues results in no logging", () => {
      ...
      const quads = [
        ...
        quad(
          subject,
          NS.pav("createdBy"),
          namedNode(buildBestuurseenheidIri(uuid()).value),
          graph,
        ),
        quad(
          subject,
          NS.schema("dateCreated"),
          literal(InstanceTestBuilder.DATE_CREATED.value),
          graph,
        ),
        quad(
          subject,
          NS.schema("dateModified"),
          literal(InstanceTestBuilder.DATE_MODIFIED.value),
          graph,
        ),
        quad(
          subject,
          NS.adms("status"),
          NS.concepts.instanceStatus(InstanceTestBuilder.STATUS),
          graph,
        ),
        quad(
          subject,
      ...
      new QuadsToDomainMapper(
        quads,
        new Iri(CONCEPT_GRAPH),
        new LoggingDoubleQuadReporter(logger),
      ).instance(instanceId);
    ...
    test("unique value contains more than one triple", () => {
      ...
      const quads = [
        ...
        quad(
          subject,
          NS.lpdcExt("dutchLanguageVariant"),
          literal(InstanceTestBuilder.DUTCH_LANGUAGE_VARIANT),
          graph,
        ),
      ];
      ...
      expect(loggerSpy).toHaveBeenCalledWith(
        `DoubleQuad|http://mu.semte.ch/graphs/public|${instanceId}|http://mu.semte.ch/vocabularies/core/uuid|undefined|1|2|"${instanceUuid}"|"${uuid2}"`,
      );
    });
    ...
    test("unique statement contains more than one triple", () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith(
        `DoubleQuad|http://mu.semte.ch/graphs/public|${instanceId}|http://purl.org/pav/createdBy|undefined|1|2|<${createdByIri1}>|<${createdByIri2}>`,
      );
    });
    ...
    test("language string contains more than one triple for same language", () => {
      ...
      const quads = [
        ...
        quad(
          subject,
          NS.dct("title"),
          literal("title nl another triple", Language.NL),
          graph,
        ),
        quad(
          subject,
          NS.dct("title"),
          literal("title nl yet another triple", Language.NL),
          graph,
        ),
      ];
      ...
      expect(loggerSpy).toHaveBeenCalledWith(
        `DoubleQuad|http://mu.semte.ch/graphs/public|${instanceId}|http://purl.org/dc/terms/title|undefined|1|3|"title nl"@nl|"title nl another triple"@nl|"title nl yet another triple"@nl`,
      );
    });
  });
    ...
    test("When not all orders are unique, throw error", () => {
      const quads = [
        ...
        quad(
          subjectCost1,
          NS.dct("title"),
          literal("title 1", Language.NL),
          graph,
        ),
        quad(
          subjectCost1,
          NS.dct("description"),
          literal("description 1", Language.NL),
          graph,
        ),
        quad(
          subjectCost1,
        ...
        quad(
          subjectCost2,
          NS.dct("title"),
          literal("title 2", Language.NL),
          graph,
        ),
        quad(
          subjectCost2,
          NS.dct("description"),
          literal("description 2", Language.NL),
          graph,
        ),
        quad(
          subjectCost2,
      ...
      const domainMapper = new QuadsToDomainMapper(
        quads,
        bestuurseenheid.userGraph(),
        new LoggingDoubleQuadReporter(logger),
      );
      expect(() => domainMapper.instance(instanceId)).toThrowWithMessage(
        SystemError,
        "Not all orders are unique",
      );
    });
