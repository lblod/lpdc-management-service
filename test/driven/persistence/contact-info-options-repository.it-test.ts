...
describe("contact info options repository", () => {
  ...
  const repository = new ContactInfoOptionsSparqlRepository(
    TEST_SPARQL_ENDPOINT,
  );

  test("can query contact info options", async () => {
    const bestuurseenheid = aBestuurseenheid().build();
    const anInstance = aFullInstance()
      .withContactPoints([
    ...
    await instanceRepository.save(
      ...
      instanceFromOtherBestuurseenheid,
    );

    const telephones = await repository.contactPointOptions(
      bestuurseenheid,
      "telephone",
    );
    expect(telephones).toEqual(["012456456", "016123123", "016456456"]);

    const emails = await repository.contactPointOptions(
      bestuurseenheid,
      "email",
    );
    ...
    const openingHours = await repository.contactPointOptions(
      bestuurseenheid,
      "openingHours",
    );
  ...
  test("invalid field name throws invariant error", async () => {
    ...
    await expect(
      repository.contactPointOptions(bestuurseenheid, "invalidfieldhandmd"),
    ).rejects.toThrowWithMessage(InvariantError, "Geen geldige veldnaam");
  });

  test("bestuurseenheid is required", async () => {
    await expect(
      repository.contactPointOptions(undefined, "telephone"),
    ).rejects.toThrowWithMessage(
      InvariantError,
      "bestuurseenheid mag niet ontbreken",
    );
  });
});
