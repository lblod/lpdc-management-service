...
describe("recursively fetches a part of a datastore into a array of quads using sparql endpoint", () => {
  ...
  const fetcher = new DatastoreToQuadsRecursiveSparqlFetcher(
    TEST_SPARQL_ENDPOINT,
  );

  test("Throws Error when endpoint cannot be reached", async () => {
    const fetcherWithIncorrectEndpoint =
      new DatastoreToQuadsRecursiveSparqlFetcher("thiscanotbereached");

    const startIri: Iri = new Iri(`http://example.com/ns#abc}`);

    await expect(
      fetcherWithIncorrectEndpoint.fetch(
        new Iri("http://some-test-graph"),
        startIri,
        [],
        [],
        [],
      ...
      Error,
      'All retries failed. Last error: Error: Invalid URI "thiscanotbereached"',
    );
  });
  ...
  test("Can query non recursively all triples linked to an IRI", async () => {
    ...
    const actualDataSet: Quad[] = await fetcher.fetch(
      new Iri("http://some-test-graph"),
      startIri,
      [],
      [],
    ...
    const expectedQuads = [
      quad(
        namedNode(startIri.value),
        namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
        namedNode("http://example.com/ns#aType"),
        namedNode(graph.value),
      ),
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#aPredicate1"),
        literal("aStringValue", "http://www.w3.org/2001/XMLSchema#string"),
        namedNode(graph.value),
      ),
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#aPredicate2"),
        literal(
          "aStringValue-no-lang",
      ...
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#aPredicate2"),
        literal("aStringValue-nl", "nl"),
        namedNode(graph.value),
      ),
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#aPredicate2"),
        literal("aStringValue-en", "en"),
        namedNode(graph.value),
      ),
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#aPredicate3"),
        literal(
          "2022-08-13T15:36:52Z",
      ...
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#aPredicate4"),
        literal("2023-10-28", "http://www.w3.org/2001/XMLSchema#date"),
        namedNode(graph.value),
      ),
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#aPredicate5"),
        literal("1200", "http://www.w3.org/2001/XMLSchema#integer"),
        namedNode(graph.value),
      ),
    ];
  ...
  test("Can query one level deep for a one-to-one relationship and retrieve all triples linked to an IRI", async () => {
    ...
    const expectedQuads = [
      ...
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#has-one-to-one"),
        namedNode(nestedIri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nestedIri.value),
        namedNode("http://example.com/ns#aPredicate1"),
        literal("aStringValue"),
        namedNode(graph.value),
      ),
    ];
    expect(actualDataSet).toEqual(expect.arrayContaining(expectedQuads));
  ...
  test("Can query one level deep for a one-to-many relationship and retrieve all triples linked to an IRI", async () => {
    ...
    const expectedQuads = [
      ...
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#has-one-to-many"),
        namedNode(nestedIri.value),
        namedNode(graph.value),
      ),
      quad(
      ...
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#has-one-to-many"),
        namedNode(nestedIri2.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nestedIri2.value),
        namedNode("http://example.com/ns#aPredicate2"),
        literal("aStringValue2"),
        namedNode(graph.value),
      ),
    ];
    expect(actualDataSet).toEqual(expect.arrayContaining(expectedQuads));
  ...
  test("Can query multiple levels deep for a one-to-one relationship and retrieve all triples linked to an IRI", async () => {
    ...
    const expectedQuads = [
      ...
      quad(
        namedNode(nestedIri.value),
        namedNode("http://example.com/ns#has-other-one-to-one"),
        namedNode(nestedNestedIri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nestedNestedIri.value),
  ...
  test("Can query one level deep for a one-to-many relationship and retrieve all triples linked to an IRI", async () => {
    ...
    const expectedQuads = [
      ...
      quad(
        namedNode(nestedIri.value),
        namedNode("http://example.com/ns#has-other-one-to-many"),
        namedNode(nestedNestedIri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nestedNestedIri.value),
        namedNode("http://example.com/ns#aPredicate1"),
        literal("aStringValue33"),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nestedIri.value),
        namedNode("http://example.com/ns#has-other-one-to-many"),
        namedNode(nested2NestedIri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nested2NestedIri.value),
        namedNode("http://example.com/ns#aPredicate1"),
        literal("aStringValue64"),
        namedNode(graph.value),
      ),
      quad(
        namedNode(startIri.value),
        namedNode("http://example.com/ns#has-one-to-many"),
        namedNode(nested2Iri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nested2Iri.value),
      ...
      quad(
        namedNode(nested2Iri.value),
        namedNode("http://example.com/ns#has-other-other-one-to-many"),
        namedNode(nestedNested2Iri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nestedNested2Iri.value),
        namedNode("http://example.com/ns#aPredicate1"),
        literal("aStringValue6687"),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nested2Iri.value),
        namedNode("http://example.com/ns#has-other-other-one-to-many"),
        namedNode(nested2nested2Iri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nested2nested2Iri.value),
        namedNode("http://example.com/ns#aPredicate1"),
        literal("aStringValue67977"),
        namedNode(graph.value),
      ),
    ];
    expect(actualDataSet).toEqual(expect.arrayContaining(expectedQuads));
  ...
  test("Can query a loop", async () => {
    ...
    const expectedQuads = [
      ...
      quad(
        namedNode(nestedNestedIri.value),
        namedNode("http://example.com/ns#has-other-other-one-to-one"),
        namedNode(startIri.value),
        namedNode(graph.value),
      ),
    ];
  ...
  test("Stops at Predicates To Stop Recursion", async () => {
    ...
    const actualDataSet: Quad[] = await fetcher.fetch(
      ...
      [],
      [
        NS.ex("predicate-to-stop-recursion").value,
        NS.ex("even-deeper-relation-to-stop-querying").value,
      ],
    ...
    const expectedQuads = [
      quad(
        namedNode(startIri.value),
        NS.ex("has-one-to-many"),
        namedNode(nestedIri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(startIri.value),
        NS.ex("predicate-to-stop-recursion"),
        namedNode(otherNestedIri.value),
        namedNode(graph.value),
      ),
      quad(
        namedNode(nestedIri.value),
        NS.ex("has-one-to-one"),
        namedNode(nestedNestedIri.value),
        namedNode(graph.value),
      ),
      quad(
      ...
      quad(
        namedNode(nestedNestedIri.value),
        NS.ex("even-deeper-relation-to-stop-querying"),
        namedNode(nestedNestedNestedIri.value),
        namedNode(graph.value),
      ),
    ];
  ...
  test("Does not query predicates that should not be queried", async () => {
    ...
    const actualDataSet: Quad[] = await fetcher.fetch(
      ...
      startIri,
      [
        NS.ex("predicate-to-not-query").value,
        NS.ex("another-predicate-to-not-query").value,
      ],
      [],
      [],
    );
  ...
  test("Recursing Into an illegal Type To Recurse Into throws error", async () => {
    ...
    await expect(
      fetcher.fetch(graph, startIri, [], [], [NS.skos("Concept").value]),
    ).rejects.toThrowWithMessage(
      SystemError,
      `Recursie in <http://www.w3.org/2004/02/skos/core#Concept> vanuit <${nestedIri.value}> is niet toegestaan`,
    );
  });
});
