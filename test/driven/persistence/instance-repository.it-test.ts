...
import {
  ...
  aMinimalInstance,
  InstanceTestBuilder,
} from "../../core/domain/instance-test-builder";
...
describe("InstanceRepository", () => {
  ...
  describe("findById", () => {
    test("When full instance exists with id, then return instance", async () => {
      ...
      const contactPoint = aFullContactPointForInstance()
        .withAddress(aFullAddressForInstance().build())
        .build();

      const anotherBestuurseenheid = aBestuurseenheid().build();
      const anotherContactPoint = aFullContactPointForInstance()
        .withAddress(aFullAddressForInstance().build())
        .build();
      ...
      const anotherInstance = aFullInstance()
        ...
        .withContactPoints([anotherContactPoint])
        .build();
      await repository.save(bestuurseenheid, anotherInstance);

      const actualInstance = await repository.findById(
        bestuurseenheid,
        instance.id,
      );
    ...
    test("When minimal instance exists with id, then return instance", async () => {
      ...
      const anotherInstance = aMinimalInstance()
        .withCreatedBy(anotherBestuurseenheid.id)
        .build();
      await repository.save(bestuurseenheid, anotherInstance);
    ...
    test("When instance does not exist with id, then throw error", async () => {
      ...
      const nonExistentInstanceId =
        InstanceBuilder.buildIri("thisiddoesnotexist");

      await expect(
        repository.findById(bestuurseenheid, nonExistentInstanceId),
      ).rejects.toThrowWithMessage(
        NotFoundError,
        `Kan <http://data.lblod.info/id/public-service/thisiddoesnotexist> niet vinden voor type <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#InstancePublicService> in graph <http://mu.semte.ch/graphs/organizations/${bestuurseenheid.uuid}/LoketLB-LPDCGebruiker>`,
      );
    });
  });
  ...
  describe("save", () => {
    ...
    test("should create publishedInstanceSnapshot when instance has status verzonden", async () => {
      ...
      const publishedInstanceSnapshotId =
        await repository.findPublishedInstanceSnapshotIdForInstance(
          bestuurseenheid,
          instance,
        );
      const publishedInstanceSnapshotQuads =
        await repository.findPublishedInstanceSnapshot(
          bestuurseenheid,
          publishedInstanceSnapshotId,
        );
      expect(publishedInstanceSnapshotQuads).toEqual(
        expect.arrayContaining([
          ...
          quad(
            ...
            namedNode(instance.createdBy.value),
            namedNode(bestuurseenheid.userGraph().value),
          ),
        ]),
      );
  ...
  describe("update", () => {
      ...
      expect(publishedInstanceSnapshotQuads).toEqual(
        expect.arrayContaining([
          ...
          quad(
            ...
            namedNode(newInstance.id.value),
            namedNode(bestuurseenheid.userGraph().value),
          ),
        ]),
      );
    ...
    test("should throw error when old instance is equal to new instance", async () => {
      ...
      ).rejects.toThrowWithMessage(SystemError, "Geen wijzigingen");
    });
  ...
  describe("delete", () => {
    test("if exists with dateSent, Removes all triples related to the instance and create tombstone triples", async () => {
      ...
      const instanceExists = await repository.exists(
        bestuurseenheid,
        instance.id,
      );
    ...
    test("if exists with dateSent, Removes all triples related to the instance and create tombstone triples using provided deletionDate", async () => {
      ...
      await repository.save(bestuurseenheid, instance);
      const tombstoneId = await repository.delete(
        bestuurseenheid,
        instance.id,
        FormatPreservingDate.of("2025-08-21T00:00:00.456545Z"),
      );
      ...
      expect(quads).toEqual(
        expect.arrayContaining([
          ...
          quad(
            ...
            namedNode("https://www.w3.org/ns/activitystreams#deleted"),
            literal(
              FormatPreservingDate.of("2025-08-21T00:00:00.456545Z").value,
              "http://www.w3.org/2001/XMLSchema#dateTime",
            ),
            namedNode(bestuurseenheid.userGraph().value),
          ),
    ...
    test("if exists without dateSent, Removes all triples related to the instance and does not create tombstone triples ", async () => {
      const bestuurseenheid = aBestuurseenheid().build();
      const instance = aFullInstance().withDateSent(undefined).build();
    ...
    test("Only the requested instance is deleted", async () => {
      ...
      await expect(
        repository.findById(bestuurseenheid, instance.id),
      ).rejects.toThrowWithMessage(
        NotFoundError,
      ...
      expect(
        await repository.findById(bestuurseenheid, anotherInstance.id),
      ).toEqual(anotherInstance);
    });
    ...
    test("When instance does not exist with id, then throw error", async () => {
      ...
      await expect(
        repository.delete(bestuurseenheid, nonExistentInstanceId),
      ).rejects.toThrowWithMessage(
        NotFoundError,
    ...
    test("if instance exists, but for other bestuurseenheid, then does not remove and throws error", async () => {
      ...
      await expect(
        repository.delete(bestuurseenheid, anotherInstance.id),
      ).rejects.toThrowWithMessage(
        NotFoundError,
      ...
      expect(
        await repository.findById(anotherBestuurseenheid, anotherInstance.id),
      ).toEqual(anotherInstance);
    });
    ...
    test("if a tombstone was once created, create a new tombstone, even if datesent is not set", async () => {
      ...
      await repository.save(bestuurseenheid, instance);
      const initialTombstoneId = await repository.delete(
        bestuurseenheid,
        instance.id,
      );
      ...
      const query = (tombstoneId: Iri): string => `
        ...
        `;
      let queryResult = await directDatabaseAccess.list(
        query(initialTombstoneId),
      );
      let quads = new SparqlQuerying().asQuads(
        queryResult,
        bestuurseenheid.userGraph().value,
      );
      ...
      const recreatedInstance = InstanceBuilder.from(instance)
        .withDateSent(undefined)
        .withStatus(InstanceStatusType.ONTWERP)
        .build();
      await repository.save(bestuurseenheid, recreatedInstance);

      let recreatedInstanceExists = await repository.exists(
        bestuurseenheid,
        recreatedInstance.id,
      );
      expect(recreatedInstanceExists).toBeTrue();

      expect(recreatedInstance.id).toEqual(instance.id);

      const nextTombstoneId = await repository.delete(
        bestuurseenheid,
        recreatedInstance.id,
      );

      recreatedInstanceExists = await repository.exists(
        bestuurseenheid,
        recreatedInstance.id,
      );
      ...
      quads = new SparqlQuerying().asQuads(
        queryResult,
        bestuurseenheid.userGraph().value,
      );
  ...
  describe("syncNeedsConversionFromFormalToInformal", () => {
    test("given formal, nl and informal instance, when choose informal, then set needsConversionFromFormalToInformal on true for formal and nl instance of that bestuurseenheid", async () => {
      ...
      await repository.syncNeedsConversionFromFormalToInformal(
        bestuurseenheid,
        ChosenFormType.INFORMAL,
      );

      const actualFormalInstance = await repository.findById(
        bestuurseenheid,
        formalInstance.id,
      );
      const actualInformalInstance = await repository.findById(
        bestuurseenheid,
        informalInstance.id,
      );
      const actualNlInstance = await repository.findById(
        bestuurseenheid,
        nlInstance.id,
      );
      ...
      const actualNlInstanceForOtherBestuurseenheid = await repository.findById(
        anotherBestuurseenheid,
      ...
      expect(actualFormalInstance.dateModified).not.toEqual(
        InstanceTestBuilder.DATE_MODIFIED,
      );
      expect(
        actualInformalInstance.needsConversionFromFormalToInformal,
      ).toBeFalse();
      expect(actualInformalInstance.dateModified).toEqual(
        InstanceTestBuilder.DATE_MODIFIED,
      );
      expect(actualNlInstance.needsConversionFromFormalToInformal).toBeTrue();
      expect(actualNlInstance.dateModified).not.toEqual(
        InstanceTestBuilder.DATE_MODIFIED,
      );
      ...
      expect(actualFormalInstanceForOtherBestuurseenheid.dateModified).toEqual(
        InstanceTestBuilder.DATE_MODIFIED,
      );
      expect(
      ...
      expect(
        actualInformalInstanceForOtherBestuurseenheid.dateModified,
      ).toEqual(InstanceTestBuilder.DATE_MODIFIED);
      expect(
        actualNlInstanceForOtherBestuurseenheid.needsConversionFromFormalToInformal,
      ).toBeFalse();
      expect(actualNlInstanceForOtherBestuurseenheid.dateModified).toEqual(
        InstanceTestBuilder.DATE_MODIFIED,
      );
    });
    ...
    test("given formal, nl  and informal instance, when choose formal, then needsConversionFromFormalToInformal remains false", async () => {
      ...
      expect(actualFormalInstance.dateModified).toEqual(
        InstanceTestBuilder.DATE_MODIFIED,
      );
      expect(
        actualInformalInstance.needsConversionFromFormalToInformal,
      ...
      expect(actualNlInstance.dateModified).toEqual(
        InstanceTestBuilder.DATE_MODIFIED,
      );
    ...
    test("when no triple exists for NeedsConversionFromFormalToInformal sync still inserts true triple", async () => {
      ...
      await directDatabaseAccess.insertData(`${bestuurseenheid.userGraph()}`, [
      ...
      expect(actualInstance.dateModified).not.toEqual(
        InstanceTestBuilder.DATE_MODIFIED,
      );
    });
  });
  ...
  describe("isPublishedToIpdc", () => {
    test("when multiple publishedInstanceSnapshots exists, and latest is published return true", async () => {
      ...
      const datePublished1 = FormatPreservingDate.of(
        "2024-01-16T00:01:00.672Z",
      );
      const sentDate2 = FormatPreservingDate.of("2024-01-17T00:00:00.672Z");
      const datePublished2 = FormatPreservingDate.of(
        "2024-01-17T00:01:00.672Z",
      );
      ...
      const publishedInstanceId1 =
        PublishedInstanceSnapshotBuilder.buildIri(uuid());
      const publishedInstanceId2 =
        PublishedInstanceSnapshotBuilder.buildIri(uuid());

      await directDatabaseAccess.insertData(bestuurseenheid.userGraph().value, [
      ...
      const actual = await repository.isPublishedToIpdc(
        bestuurseenheid,
        instance,
      );
  ...
  describe("Verify ontology and mapping", () => {
    test("verify saved instance is same as loaded", async () => {
      ...
      await repository.save(bestuurseenheid, instance);
      const savedInstance = await repository.findById(
        bestuurseenheid,
        instance.id,
      );
    ...
    test("Verify minimal mappings - invalid dutch language should throw error", async () => {
      ...
      await expect(() =>
        repository.findById(bestuurseenheid, instanceId),
      ).rejects.toThrowWithMessage(
        InvariantError,
    ...
    test("Verify minimal mappings - non-existing language as dutch language should throw error", async () => {
      ...
      await expect(() =>
        ...
        SystemError,
        `Kan '<${instanceId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#dutchLanguageVariant> "blabla" .' niet mappen.`,
      );
    });
    ...
    test("Verify minimal mappings - requirement without evidence", async () => {
      ...
      const actualConceptSnapshot = await repository.findById(
        bestuurseenheid,
        instance.id,
      );
    ...
    test("Unknown Instance Status Type can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${instanceId}> <http://www.w3.org/ns/adms#status> <http://lblod.data.gift/concepts/instance-status/unknown-instance-status> .' niet mappen.`,
      );
    });
    ...
    test("Unknown ProductType can not be mapped", async () => {
      ...
      await expect(
        repository.findById(bestuurseenheid, instanceIri),
      ).rejects.toThrowWithMessage(
        SystemError,
        `Kan '<${instanceIri}> <http://purl.org/dc/terms/type> <https://productencatalogus.data.vlaanderen.be/id/concept/Type/UnknownProductType> .' niet mappen.`,
      );
    });
    ...
    for (const targetAudience of Object.values(TargetAudienceType)) {
      test(`TargetAudienceType ${targetAudience} can be mapped`, async () => {
        const bestuurseenheid = aBestuurseenheid().build();
        const instance = aMinimalInstance()
          .withTargetAudiences([targetAudience])
          .build();
        await repository.save(bestuurseenheid, instance);
    ...
    test("Unknown Target Audience Type can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${instanceIri}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#targetAudience> <https://productencatalogus.data.vlaanderen.be/id/concept/Doelgroep/NonExistingTargetAudience> .' niet mappen.`,
      );
    });
    ...
    test("Unknown Theme type can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${instanceIri}> <http://data.europa.eu/m8g/thematicArea> <https://productencatalogus.data.vlaanderen.be/id/concept/Thema/NonExistingTheme> .' niet mappen.`,
      );
    });

    for (const competentAuthorityLevel of Object.values(
      CompetentAuthorityLevelType,
    )) {
    ...
    test("Unknown Competent Authority Level type can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${instanceIri}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#competentAuthorityLevel> <https://productencatalogus.data.vlaanderen.be/id/concept/BevoegdBestuursniveau/NonExistingCompetentAuthorityLevel> .' niet mappen.`,
      );
    });

    for (const executingAuthorityLevel of Object.values(
      ExecutingAuthorityLevelType,
    )) {
    ...
    test("Unknown ExecutingAuthorityLevelType can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${instanceIri}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#executingAuthorityLevel> <https://productencatalogus.data.vlaanderen.be/id/concept/UitvoerendBestuursniveau/NonExistingExecutingAuthorityLevel> .' niet mappen.`,
      );
    });
    ...
    test("Unknown PublicationMediumType can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${instanceId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#publicationMedium> <https://productencatalogus.data.vlaanderen.be/id/concept/PublicatieKanaal/NonExistingPublicationMedium> .' niet mappen.`,
      );
    });
    ...
    test("Unknown YourEuropeCategoryType can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${instanceId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#yourEuropeCategory> <https://productencatalogus.data.vlaanderen.be/id/concept/YourEuropeCatagory/NonExistingYourEuropeCategory> .' niet mappen.`,
      );
    });
    ...
    test("Unknown LanguageType can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${instanceId}> <http://purl.org/dc/terms/language> <http://publications.europa.eu/resource/authority/language/NonExistingLanguageType> .' niet mappen.`,
      );
    });
    ...
    for (const language of instanceLanguages) {
      test(`DutchLanguageVariant Language ${language} can be mapped`, async () => {
        const bestuurseenheid = aBestuurseenheid().build();
        const instance = aMinimalInstance()
          .withDutchLanguageVariant(language)
          .build();
        await repository.save(bestuurseenheid, instance);
    ...
    test("verify mapping publishedInstanceSnapshot", () => {
      ...
      const publishedInstanceSnapshot =
        PublishedInstanceSnapshotBuilder.from(instance);
      const quads = new DomainToQuadsMapper(
        bestuurseenheid.userGraph(),
      ).publishedInstanceSnapshotToQuads(publishedInstanceSnapshot);
      const expectedQuads = [
        quad(
        ...
        quad(
          ...
          namedNode("http://www.w3.org/ns/prov#generatedAtTime"),
          literal(
            publishedInstanceSnapshot.generatedAtTime.value,
            "http://www.w3.org/2001/XMLSchema#dateTime",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://purl.org/pav/createdBy"),
          namedNode(publishedInstanceSnapshot.createdBy.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://purl.org/dc/terms/title"),
          literal(publishedInstanceSnapshot.title.nlFormal, Language.FORMAL),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          ...
          literal(
            publishedInstanceSnapshot.description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(
            publishedInstanceSnapshot.additionalDescription.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(
            publishedInstanceSnapshot.exception.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(
            publishedInstanceSnapshot.regulation.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://schema.org/startDate"),
          literal(
            publishedInstanceSnapshot.startDate.value,
            "http://www.w3.org/2001/XMLSchema#dateTime",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://schema.org/endDate"),
          literal(
            publishedInstanceSnapshot.endDate.value,
            "http://www.w3.org/2001/XMLSchema#dateTime",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(NS.dvc.type(publishedInstanceSnapshot.type).value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          ...
          namedNode(
            NS.dvc.doelgroep(publishedInstanceSnapshot.targetAudiences[0])
              .value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            NS.dvc.doelgroep(publishedInstanceSnapshot.targetAudiences[1])
              .value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            NS.dvc.doelgroep(publishedInstanceSnapshot.targetAudiences[2])
              .value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/thematicArea"),
          namedNode(NS.dvc.thema(publishedInstanceSnapshot.themes[0]).value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/thematicArea"),
          namedNode(NS.dvc.thema(publishedInstanceSnapshot.themes[1]).value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/thematicArea"),
          namedNode(NS.dvc.thema(publishedInstanceSnapshot.themes[2]).value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          ...
          namedNode(
            NS.dvc.bevoegdBestuursniveau(
              publishedInstanceSnapshot.competentAuthorityLevels[0],
            ).value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/hasCompetentAuthority"),
          namedNode(publishedInstanceSnapshot.competentAuthorities[1].value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/hasCompetentAuthority"),
          namedNode(publishedInstanceSnapshot.competentAuthorities[2].value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
        ...
        quad(
          ...
          namedNode(publishedInstanceSnapshot.executingAuthorities[0].value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          ...
          namedNode(publishedInstanceSnapshot.executingAuthorities[1].value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          ...
          namedNode(
            NS.dvc.publicatieKanaal(
              publishedInstanceSnapshot.publicationMedia[0],
            ).value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            NS.dvc.publicatieKanaal(
              publishedInstanceSnapshot.publicationMedia[1],
            ).value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            NS.dvc.yourEuropeCategorie(
              publishedInstanceSnapshot.yourEuropeCategories[0],
            ).value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            NS.dvc.yourEuropeCategorie(
              publishedInstanceSnapshot.yourEuropeCategories[1],
            ).value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            NS.dvc.yourEuropeCategorie(
              publishedInstanceSnapshot.yourEuropeCategories[2],
            ).value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://www.w3.org/ns/dcat#keyword"),
          literal(publishedInstanceSnapshot.keywords[0].nl, Language.NL),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://www.w3.org/ns/dcat#keyword"),
          literal(publishedInstanceSnapshot.keywords[1].nl, Language.NL),
          namedNode(bestuurseenheid.userGraph().value),
        ),

        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://vocab.belgif.be/ns/publicservice#hasRequirement"),
          namedNode(publishedInstanceSnapshot.requirements[0].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.requirements[0].id.value),
          namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
          namedNode("http://data.europa.eu/m8g/Requirement"),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.requirements[0].id.value),
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.requirements[0].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.requirements[0].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(
            publishedInstanceSnapshot.requirements[0].order.toString(),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.requirements[0].id.value),
          namedNode("http://data.europa.eu/m8g/hasSupportingEvidence"),
          namedNode(
            publishedInstanceSnapshot.requirements[0].evidence.id.value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
          namedNode("http://data.europa.eu/m8g/Evidence"),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.requirements[0].evidence.title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.requirements[0].evidence.description
              .nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://vocab.belgif.be/ns/publicservice#hasRequirement"),
          namedNode(publishedInstanceSnapshot.requirements[1].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.requirements[1].id.value),
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.requirements[1].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.requirements[1].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.requirements[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://data.europa.eu/m8g/hasSupportingEvidence"),
          namedNode(
            publishedInstanceSnapshot.requirements[1].evidence.id.value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.requirements[1].evidence.title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.requirements[1].evidence.description
              .nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://purl.org/vocab/cpsv#follows"),
          namedNode(publishedInstanceSnapshot.procedures[0].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.procedures[0].id.value),
          namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
          namedNode("http://purl.org/vocab/cpsv#Rule"),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.procedures[0].id.value),
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.procedures[0].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.procedures[0].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.procedures[0].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            publishedInstanceSnapshot.procedures[0].websites[0].id.value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
          namedNode("http://schema.org/WebSite"),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.procedures[0].websites[0].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.procedures[0].websites[0].description
              .nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ...
        quad(
          ...
          literal(publishedInstanceSnapshot.procedures[0].websites[0].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.procedures[0].websites[0].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            publishedInstanceSnapshot.procedures[0].websites[1].id.value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.procedures[0].websites[1].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.procedures[0].websites[1].description
              .nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ...
        quad(
          ...
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.procedures[0].websites[1].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.procedures[0].websites[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://purl.org/vocab/cpsv#follows"),
          namedNode(publishedInstanceSnapshot.procedures[1].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.procedures[1].id.value),
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.procedures[1].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.procedures[1].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.procedures[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            publishedInstanceSnapshot.procedures[1].websites[0].id.value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.procedures[1].websites[0].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.procedures[1].websites[0].description
              .nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ...
        quad(
          ...
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.procedures[1].websites[0].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.procedures[1].websites[0].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            publishedInstanceSnapshot.procedures[1].websites[1].id.value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.procedures[1].websites[1].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.procedures[1].websites[1].description
              .nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ...
        quad(
          ...
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.procedures[1].websites[1].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.procedures[1].websites[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          ...
          namedNode(publishedInstanceSnapshot.websites[0].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.websites[0].id.value),
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.websites[0].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.websites[0].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.websites[0].id.value),
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.websites[0].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.websites[0].id.value),
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.websites[0].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://www.w3.org/2000/01/rdf-schema#seeAlso"),
          namedNode(publishedInstanceSnapshot.websites[1].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.websites[1].id.value),
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.websites[1].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.websites[1].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.websites[1].id.value),
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.websites[1].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.websites[1].id.value),
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.websites[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/hasCost"),
          namedNode(publishedInstanceSnapshot.costs[0].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.costs[0].id.value),
          namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
          namedNode("http://data.europa.eu/m8g/Cost"),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.costs[0].id.value),
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.costs[0].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.costs[0].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.costs[0].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/hasCost"),
          namedNode(publishedInstanceSnapshot.costs[1].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.costs[1].id.value),
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.costs[1].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.costs[1].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.costs[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://purl.org/vocab/cpsv#produces"),
          namedNode(publishedInstanceSnapshot.financialAdvantages[0].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.financialAdvantages[0].id.value),
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.financialAdvantages[0].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.financialAdvantages[0].description
              .nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ...
        quad(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.financialAdvantages[0].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://purl.org/vocab/cpsv#produces"),
          namedNode(publishedInstanceSnapshot.financialAdvantages[1].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.financialAdvantages[1].id.value),
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.financialAdvantages[1].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.financialAdvantages[1].description
              .nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ...
        quad(
          ...
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.financialAdvantages[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/hasContactPoint"),
          namedNode(publishedInstanceSnapshot.contactPoints[0].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[0].id.value),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[0].id.value),
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.contactPoints[0].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[0].id.value),
          namedNode("http://schema.org/email"),
          literal(publishedInstanceSnapshot.contactPoints[0].email),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[0].id.value),
          namedNode("http://schema.org/telephone"),
          literal(publishedInstanceSnapshot.contactPoints[0].telephone),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[0].id.value),
          namedNode("http://schema.org/openingHours"),
          literal(publishedInstanceSnapshot.contactPoints[0].openingHours),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[0].id.value),
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.contactPoints[0].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            publishedInstanceSnapshot.contactPoints[0].address.id.value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
          namedNode("http://www.w3.org/ns/locn#Address"),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#gemeentenaam"),
          literal(
            publishedInstanceSnapshot.contactPoints[0].address.gemeentenaam.nl,
            Language.NL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#land"),
          literal(
            publishedInstanceSnapshot.contactPoints[0].address.land.nl,
            Language.NL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(
            publishedInstanceSnapshot.contactPoints[0].address.huisnummer,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(publishedInstanceSnapshot.contactPoints[0].address.busnummer),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#postcode"),
          literal(publishedInstanceSnapshot.contactPoints[0].address.postcode),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#Straatnaam"),
          literal(
            publishedInstanceSnapshot.contactPoints[0].address.straatnaam.nl,
            Language.NL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#verwijstNaar"),
          namedNode(
            publishedInstanceSnapshot.contactPoints[0].address.verwijstNaar
              .value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/hasContactPoint"),
          namedNode(publishedInstanceSnapshot.contactPoints[1].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[1].id.value),
        ...
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[1].id.value),
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.contactPoints[1].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[1].id.value),
          namedNode("http://schema.org/email"),
          literal(publishedInstanceSnapshot.contactPoints[1].email),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[1].id.value),
          namedNode("http://schema.org/telephone"),
          literal(publishedInstanceSnapshot.contactPoints[1].telephone),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[1].id.value),
          namedNode("http://schema.org/openingHours"),
          literal(publishedInstanceSnapshot.contactPoints[1].openingHours),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.contactPoints[1].id.value),
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.contactPoints[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode(
            publishedInstanceSnapshot.contactPoints[1].address.id.value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
        ...
        quad(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#gemeentenaam"),
          literal(
            publishedInstanceSnapshot.contactPoints[1].address.gemeentenaam.nl,
            Language.NL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#land"),
          literal(
            publishedInstanceSnapshot.contactPoints[1].address.land.nl,
            Language.NL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(
            publishedInstanceSnapshot.contactPoints[1].address.huisnummer,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#postcode"),
          literal(publishedInstanceSnapshot.contactPoints[1].address.postcode),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#Straatnaam"),
          literal(
            publishedInstanceSnapshot.contactPoints[1].address.straatnaam.nl,
            Language.NL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("https://data.vlaanderen.be/ns/adres#verwijstNaar"),
          namedNode(
            publishedInstanceSnapshot.contactPoints[1].address.verwijstNaar
              .value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        ...
        quad(
          ...
          namedNode(publishedInstanceSnapshot.conceptId.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          ...
          namedNode(
            NS.pera.languageType(publishedInstanceSnapshot.languages[0]).value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/language"),
          namedNode(
            NS.pera.languageType(publishedInstanceSnapshot.languages[1]).value,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(
            publishedInstanceSnapshot.dateCreated.value,
            "http://www.w3.org/2001/XMLSchema#dateTime",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          literal(
            publishedInstanceSnapshot.dateModified.value,
            "http://www.w3.org/2001/XMLSchema#dateTime",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://purl.org/dc/terms/spatial"),
          namedNode(publishedInstanceSnapshot.spatials[0].value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://purl.org/dc/terms/spatial"),
          namedNode(publishedInstanceSnapshot.spatials[1].value),
          namedNode(bestuurseenheid.userGraph().value),
        ),

        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/hasLegalResource"),
          namedNode(publishedInstanceSnapshot.legalResources[0].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.legalResources[0].id.value),
          namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"),
          namedNode("http://data.europa.eu/eli/ontology#LegalResource"),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.legalResources[0].id.value),
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.legalResources[0].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.legalResources[0].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.legalResources[0].id.value),
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.legalResources[0].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.legalResources[0].id.value),
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.legalResources[0].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.id.value),
          namedNode("http://data.europa.eu/m8g/hasLegalResource"),
          namedNode(publishedInstanceSnapshot.legalResources[1].id.value),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.legalResources[1].id.value),
        ...
        quad(
          ...
          namedNode("http://purl.org/dc/terms/title"),
          literal(
            publishedInstanceSnapshot.legalResources[1].title.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          ...
          namedNode("http://purl.org/dc/terms/description"),
          literal(
            publishedInstanceSnapshot.legalResources[1].description.nlFormal,
            Language.FORMAL,
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.legalResources[1].id.value),
          namedNode("http://schema.org/url"),
          literal(publishedInstanceSnapshot.legalResources[1].url),
          namedNode(bestuurseenheid.userGraph().value),
        ),
        quad(
          namedNode(publishedInstanceSnapshot.legalResources[1].id.value),
          namedNode("http://www.w3.org/ns/shacl#order"),
          literal(
            publishedInstanceSnapshot.legalResources[1].order.toString(),
            "http://www.w3.org/2001/XMLSchema#integer",
          ),
          namedNode(bestuurseenheid.userGraph().value),
        ),
