...
import {
  ...
  aMinimalConcept,
  ConceptTestBuilder,
} from "../../core/domain/concept-test-builder";
...
describe("ConceptRepository", () => {
  ...
  describe("findById", () => {
    ...
    test("When minimal concept with incomplete title exists with id, then return concept", async () => {
      const concept = aMinimalConcept()
        .withTitle(LanguageString.of(ConceptTestBuilder.TITLE_NL))
        .build();
      await repository.save(concept);
    ...
    test("When concept does not exist with id, then throw error", async () => {
      ...
      await expect(
        repository.findById(nonExistentConceptId),
      ).rejects.toThrowWithMessage(
        NotFoundError,
        `Kan <https://ipdc.tni-vlaanderen.be/id/concept/thisiddoesnotexist> niet vinden voor type <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#ConceptualPublicService> in graph <http://mu.semte.ch/graphs/public>`,
      );
    });
  });
  ...
  describe("update", () => {
    test("should update concept", async () => {
      ...
      const newConcept = aFullConcept()
        .withEndDate(FormatPreservingDate.now())
        .build();
    ...
    test("should throw error when old concept is equal to new concept", async () => {
      ...
      await expect(() =>
        repository.update(newConcept, oldConcept),
  ...
  describe("Verify ontology and mapping", () => {
    test("Verify correct type", async () => {
      ...
      await directDatabaseAccess.insertData(CONCEPT_GRAPH, [
      ...
      await expect(
        ...
        NotFoundError,
        `Kan <${idForIncorrectType}> niet vinden voor type <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#ConceptualPublicService>, maar wel gevonden met type <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#SomeUnkownType> in graph <http://mu.semte.ch/graphs/public>`,
      );
    });
