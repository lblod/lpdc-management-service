...
describe("ConceptDisplayConfigurationRepository", () => {
  ...
  const repository = new ConceptDisplayConfigurationSparqlTestRepository(
  ...
  describe("findById", () => {
    test("Is returned when found", async () => {
      ...
      const actualConceptDisplayConfiguration = await repository.findById(
        bestuurseenheid,
        conceptDisplayConfiguration.id,
      );
      expect(actualConceptDisplayConfiguration).toEqual(
        conceptDisplayConfiguration,
      );
    });
    ...
    test("throws error when not found", async () => {
      ...
      const anotherConceptDisplayConfigurationId =
        buildConceptDisplayConfigurationIri(uuid());

      await expect(() =>
        repository.findById(
          bestuurseenheid,
          anotherConceptDisplayConfigurationId,
        ),
      ).rejects.toThrowWithMessage(
        NotFoundError,
        `Geen conceptDisplayConfiguratie gevonden voor id: ${anotherConceptDisplayConfigurationId}`,
      );
    });
    ...
    test("only searches in configured bestuurseenheid graph of bestuurseenheid", async () => {
      ...
      await expect(() =>
        repository.findById(bestuurseenheid, conceptDisplayConfiguration.id),
      ).rejects.toThrowWithMessage(
        SystemError,
        `Concept display configuration ${conceptDisplayConfiguration.id} gevonden in de foute gebruikers graph`,
      );
    });
  });
  ...
  describe("findByConceptId", () => {
    test("Is returned when found", async () => {
      ...
      const actualConceptDisplayConfiguration =
        await repository.findByConceptId(
          bestuurseenheid,
          conceptDisplayConfiguration.conceptId,
        );
      expect(actualConceptDisplayConfiguration).toEqual(
        conceptDisplayConfiguration,
      );
    ...
    test("throws error when not found", async () => {
      ...
      await expect(() =>
        repository.findByConceptId(bestuurseenheid, anotherConceptId),
      ).rejects.toThrowWithMessage(
        NotFoundError,
        `Geen conceptDisplayConfiguration gevonden voor bestuurseenheid: ${bestuurseenheid.id} en concept ${anotherConceptId}`,
      );
    });
    ...
    test("filters on concept id", async () => {
      ...
      await repository.save(
        bestuurseenheid,
        anotherConceptDisplayConfiguration,
      );
    ...
    test("only searches in configured bestuurseenheid graph of bestuurseenheid", async () => {
      ...
      await expect(() =>
        repository.findByConceptId(bestuurseenheid, conceptId),
      ).rejects.toThrowWithMessage(
        SystemError,
  ...
  describe("removeConceptIsNewFlag", () => {
    test("When conceptIsNew is true", async () => {
      ...
      await repository.removeConceptIsNewFlag(
        bestuurseenheid,
        conceptDisplayConfiguration.id,
      );
      ...
      const expectedConceptDisplayConfiguration =
        ConceptDisplayConfigurationBuilder.from(conceptDisplayConfiguration)
          .withConceptIsNew(false)
          .build();

      expect(actualConceptDisplayConfiguration).toEqual(
        expectedConceptDisplayConfiguration,
      );
    });
    ...
    test("if concept-displayConfig does not exists, throws error", async () => {
      const bestuurseenheid = aBestuurseenheid().build();
      const conceptDisplayConfigurationIri =
        buildConceptDisplayConfigurationIri(uuid());

      await expect(
        repository.removeConceptIsNewFlag(
          bestuurseenheid,
          conceptDisplayConfigurationIri,
        ),
      ).rejects.toThrowWithMessage(
        NotFoundError,
        `Geen conceptDisplayConfiguratie gevonden voor id: ${conceptDisplayConfigurationIri}`,
      );
    });
  });
  ...
  describe("syncInstantiatedFlag", () => {
    test("When instance exists for concept, Then conceptIsInstantiated is true and conceptIsNew false", async () => {
      ...
      expect(actualConceptDisplayConfiguration.conceptIsInstantiated).toEqual(
        true,
      );
    ...
    test("When no instance exists for concept & conceptIsNew false & conceptIsInstantiated true, Then conceptIsInstantiated is false and conceptIsNew is false", async () => {
      ...
      await repository.syncInstantiatedFlag(
        bestuurseenheid,
        conceptDisplayConfiguration.conceptId,
      );
    ...
    test("if concept-display-configuration does not exists, throws error", async () => {
      ...
      await expect(
        repository.syncInstantiatedFlag(bestuurseenheid, conceptId),
      ).rejects.toThrowWithMessage(
        NotFoundError,
  ...
  describe("Verify ontology and mapping", () => {
    test("Verify correct type", async () => {
      ...
      await directDatabaseAccess.insertData(
        ...
        [PREFIX.lpdc, PREFIX.mu, PREFIX.dct],
      );

      await expect(() =>
        repository.findByConceptId(
          bestuurseenheid,
          conceptDisplayConfiguration.conceptId,
        ),
      ).rejects.toThrowWithMessage(
        NotFoundError,
        `Geen conceptDisplayConfiguration gevonden voor bestuurseenheid: ${bestuurseenheid.id} en concept ${conceptDisplayConfiguration.conceptId}`,
      );
    });
  ...
  describe("ensure concept display configurations for all bestuurseenheden", () => {
    test("Creates new concept display configurations for a concept if not existing for bestuurseenheid", async () => {
      ...
      await repository.ensureConceptDisplayConfigurationsForAllBestuurseenheden(
        concept.id,
      );

      const createdConceptDisplayConfigurationForBestuurseenheid =
        await repository.findByConceptId(bestuurseenheid, concept.id);
      expect(
      ...
      const createdConceptDisplayConfigurationForAnotherBestuurseenheid =
        await repository.findByConceptId(anotherBestuurseenheid, concept.id);
      expect(
    ...
    test("Does not create new or update concept display configurations for a concept if already exists for bestuurseenheid", async () => {
      ...
      const createdConceptDisplayConfigurationForBestuurseenheid =
        await repository.findByConceptId(bestuurseenheid, concept.id);
      expect(createdConceptDisplayConfigurationForBestuurseenheid.id).toEqual(
        conceptDisplayConfiguration.id,
      );
      expect(createdConceptDisplayConfigurationForBestuurseenheid.uuid).toEqual(
        conceptDisplayConfiguration.uuid,
      );
      expect(
        createdConceptDisplayConfigurationForBestuurseenheid.conceptIsNew,
      ).toEqual(false);
