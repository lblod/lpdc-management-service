...
import {
  ...
  EvidenceTestBuilder,
} from "../../core/domain/evidence-test-builder";
import {
...
describe("ConceptSnapshotRepository", () => {
  const repository = new ConceptSnapshotSparqlTestRepository(
  ...
  describe("findById", () => {
    ...
    test("When minimal concept snapshot with incomplete title exists with id, then return concept snapshot", async () => {
      const conceptSnapshot = aMinimalConceptSnapshot()
        .withTitle(LanguageString.of(ConceptSnapshotTestBuilder.TITLE_NL))
        .build();
      await repository.save(conceptSnapshot);
    ...
    test("When concept snapshot does not exist with id, then throw error", async () => {
      ...
      const nonExistentConceptSnapshotId =
        buildConceptSnapshotIri("thisiddoesnotexist");

      await expect(
        repository.findById(nonExistentConceptSnapshotId),
      ).rejects.toThrowWithMessage(
        NotFoundError,
        `Kan <https://ipdc.tni-vlaanderen.be/id/conceptsnapshot/thisiddoesnotexist> niet vinden voor type <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#ConceptualPublicServiceSnapshot> in graph <http://mu.semte.ch/graphs/lpdc/conceptsnapshots-ldes-data/ipdc>`,
      );
    });
  });
  ...
  describe("Verify ontology and mapping", () => {
    test("Verify correct type", async () => {
      const idForIncorrectType = new Iri(
        `https://ipdc.tni-vlaanderen.be/id/rule/${uuid()}`,
      );

      await directDatabaseAccess.insertData(CONCEPT_SNAPSHOT_LDES_GRAPH, [
        `<${idForIncorrectType}> a <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#SomeUnkownType>`,
      ]);

      await expect(
        repository.findById(idForIncorrectType),
      ).rejects.toThrowWithMessage(
        NotFoundError,
        `Kan <${idForIncorrectType}> niet vinden voor type <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#ConceptualPublicServiceSnapshot>, maar wel gevonden met type <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#SomeUnkownType> in graph <http://mu.semte.ch/graphs/lpdc/conceptsnapshots-ldes-data/ipdc>`,
      );
    });
    ...
    test("Verify minimal mappings", async () => {
      ...
      const conceptSnapshotDescription =
        aMinimalLanguageString("description").build();
      const conceptSnapshotProductId = ConceptSnapshotTestBuilder.PRODUCT_ID;
      const conceptSnapshotDateCreated =
        ConceptSnapshotTestBuilder.DATE_CREATED;
      const conceptSnapshotDateModified =
        ConceptSnapshotTestBuilder.DATE_MODIFIED;
      const conceptSnapshotGeneratedAtTime =
        ConceptSnapshotTestBuilder.GENERATED_AT_TIME;
      const conceptSnapshotIsVersionOf =
        ConceptSnapshotTestBuilder.IS_VERSION_OF_CONCEPT;
      ...
      const actualConceptSnapshot =
        await repository.findById(conceptSnapshotId);
    ...
    test("Verify minimal mappings - requirement without evidence", async () => {
      const conceptSnapshotId = buildConceptSnapshotIri(uuid());
      const conceptSnapshotTitle = ConceptSnapshotTestBuilder.MINIMAL_TITLE;
      const conceptSnapshotDescription = aMinimalLanguageString(
        ConceptSnapshotTestBuilder.DESCRIPTION,
      ).build();
      const conceptSnapshotProductId = ConceptSnapshotTestBuilder.PRODUCT_ID;
      const conceptSnapshotDateCreated =
        ConceptSnapshotTestBuilder.DATE_CREATED;
      ...
      const conceptSnapshotGeneratedAtTime =
        ConceptSnapshotTestBuilder.GENERATED_AT_TIME;
      const conceptSnapshotIsVersionOfConcept =
        ConceptSnapshotTestBuilder.IS_VERSION_OF_CONCEPT;

      const requirementId = RequirementBuilder.buildIri(uuid());
      const requirementTitle = aMinimalLanguageString(
        RequirementTestBuilder.TITLE,
      ).build();
      const requirementDescription = aMinimalLanguageString(
        ConceptSnapshotTestBuilder.DESCRIPTION,
      ).build();
    ...
    test("Verify minimal mappings - minimal requirement with minimal evidence", async () => {
      ...
      const evidenceTitle = aMinimalLanguageString(
        EvidenceTestBuilder.TITLE,
      ).build();
      const evidenceDescription = aMinimalLanguageString(
        EvidenceTestBuilder.DESCRIPTION,
      ).build();
    ...
    test("Verify minimal mappings - procedure without websites", async () => {
      ...
      const procedureTitle = aMinimalLanguageString(
        ProcedureTestBuilder.TITLE,
      ).build();
      const procedureDescription = aMinimalLanguageString(
        ProcedureTestBuilder.DESCRIPTION,
      ).build();
    ...
    test("Verify minimal mappings - procedure with minimal website", async () => {
      ...
      const websiteTitle = aMinimalLanguageString(
        WebsiteTestBuilder.TITLE,
      ).build();
      ...
      describe("for websites", () => {
        for (const nonDutchLanguage of ["de", "fr", "en"]) {
          test(`Filters out websites when title only available in language ${nonDutchLanguage}`, async () => {
            const conceptSnapshotId = buildConceptSnapshotIri(uuid());
            const conceptSnapshotTitle =
              ConceptSnapshotTestBuilder.MINIMAL_TITLE;
            const conceptSnapshotDescription =
              aMinimalLanguageString("description").build();
            const conceptSnapshotProductId =
              ConceptSnapshotTestBuilder.PRODUCT_ID;
            const conceptSnapshotDateCreated =
              ConceptSnapshotTestBuilder.DATE_CREATED;
            const conceptSnapshotDateModified =
              ConceptSnapshotTestBuilder.DATE_MODIFIED;
            const conceptSnapshotGeneratedAtTime =
              ConceptSnapshotTestBuilder.GENERATED_AT_TIME;
            const conceptSnapshotRetainedWebsite = anotherFullWebsite(uuid())
              .withOrder(2)
              .build();
            const conceptSnapshotNotRetainedWebsiteId =
        ...
        test(`Throws error if required fields are missing on website`, async () => {
          ...
          await expect(
            repository.findById(conceptSnapshotId),
          ).rejects.toThrowWithMessage(
            InvariantError,
            `order mag niet ontbreken`,
          );
        });
      });
        ...
        for (const nonDutchLanguage of ["de", "fr", "en"]) {
          test(`Filters out websites when title only available in language ${nonDutchLanguage}`, async () => {
            ...
            const conceptSnapshotRetainedWebsite = anotherFullWebsite(uuid())
              .withOrder(2)
              .build();
            const conceptSnapshotProcedure = anotherFullProcedure()
              .withOrder(1)
              .withWebsites([conceptSnapshotRetainedWebsite])
              .build();
            const conceptSnapshotNotRetainedWebsiteId =
        ...
        for (const nonDutchLanguage of ["de", "fr", "en"]) {
          test(`Filters out procedures when title only available in language ${nonDutchLanguage}`, async () => {
            ...
            const conceptSnapshotProcedure = anotherFullProcedure()
              ...
              .withWebsites([conceptSnapshotRetainedWebsite])
              .build();
            const conceptSnapshotNotRetainedProcedureId =
      ...
      describe("for requirements", () => {
        for (const nonDutchLanguage of ["de", "fr", "en"]) {
          test(`Filters out requirement when title only available in language ${nonDutchLanguage}`, async () => {
            ...
            const conceptSnapshotGeneratedAtTime =
              ConceptSnapshotTestBuilder.GENERATED_AT_TIME;
            const conceptSnapshotRequirement = anotherFullRequirement()
              .withOrder(2)
              .build();
            const conceptSnapshotNotRetainedRequirementId =
        ...
        test(`Throws Error if required fields are missing for requirements`, async () => {
          ...
          const conceptSnapshotGeneratedAtTime =
            ConceptSnapshotTestBuilder.GENERATED_AT_TIME;
          const conceptSnapshotRequirementId =
            RequirementBuilder.buildIri(uuid());
          const conceptSnapshotIsVersionOf =
            ConceptSnapshotTestBuilder.IS_VERSION_OF_CONCEPT;
        ...
        for (const nonDutchLanguage of ["de", "fr", "en"]) {
          test(`Filters out evidence when title only available in language ${nonDutchLanguage}`, async () => {
            ...
            const conceptSnapshotRequirement = anotherFullRequirement()
              ...
              .withOrder(2)
              .build();
            const conceptSnapshotNotRetainedEvidenceId =
              EvidenceBuilder.buildIri(uuid());
            const conceptSnapshotIsVersionOf =
              ConceptSnapshotTestBuilder.IS_VERSION_OF_CONCEPT;
      ...
      describe("for costs", () => {
        for (const nonDutchLanguage of ["de", "fr", "en"]) {
          test(`Filters out costs when title only available in language ${nonDutchLanguage}`, async () => {
            ...
            const conceptSnapshotNotRetainedCostId =
      ...
      describe("for financial advantages", () => {
        for (const nonDutchLanguage of ["de", "fr", "en"]) {
          test(`Filters out financial advantages when title only available in language ${nonDutchLanguage}`, async () => {
            ...
            const conceptSnapshotGeneratedAtTime =
              ConceptSnapshotTestBuilder.GENERATED_AT_TIME;
            const conceptSnapshotFinancialAdvantage =
              anotherFullFinancialAdvantage().withOrder(2).build();
            const conceptSnapshotNotRetainedFinancialAdvantageId =
              CostBuilder.buildIri(uuid());
            const conceptSnapshotIsVersionOf =
              ConceptSnapshotTestBuilder.IS_VERSION_OF_CONCEPT;
        ...
        test(`Throws Error if required fields are missing for financial advantages`, async () => {
          ...
          const conceptSnapshotGeneratedAtTime =
            ConceptSnapshotTestBuilder.GENERATED_AT_TIME;
          const conceptSnapshotFinancialAdvantageId =
    ...
    test("Unknown ProductType can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${conceptSnapshotId}> <http://purl.org/dc/terms/type> <https://productencatalogus.data.vlaanderen.be/id/concept/Type/UnknownProductType> .' niet mappen.`,
      );
    });
    ...
    test("Unknown Target Audience Type can not be mapped", async () => {
      const conceptSnapshotId = new Iri(
        `https://ipdc.tni-vlaanderen.be/id/conceptsnapshot/${uuid()}`,
      );
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${conceptSnapshotId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#targetAudience> <https://productencatalogus.data.vlaanderen.be/id/concept/Doelgroep/NonExistingTargetAudience> .' niet mappen.`,
      );
    });
    ...
    test("Unknown Theme type can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${conceptSnapshotId}> <http://data.europa.eu/m8g/thematicArea> <https://productencatalogus.data.vlaanderen.be/id/concept/Thema/NonExistingTheme> .' niet mappen.`,
      );
    });
    ...
    test("Unknown Competent Authority Level type can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${conceptSnapshotId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#competentAuthorityLevel> <https://productencatalogus.data.vlaanderen.be/id/concept/BevoegdBestuursniveau/NonExistingCompetentAuthorityLevel> .' niet mappen.`,
      );
    });
    ...
    test("Unknown ExecutingAuthorityLevelType can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${conceptSnapshotId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#executingAuthorityLevel> <https://productencatalogus.data.vlaanderen.be/id/concept/UitvoerendBestuursniveau/NonExistingExecutingAuthorityLevel> .' niet mappen.`,
      );
    });
    ...
    test("Unknown PublicationMediumType can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${conceptSnapshotId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#publicationMedium> <https://productencatalogus.data.vlaanderen.be/id/concept/PublicatieKanaal/NonExistingPublicationMedium> .' niet mappen.`,
      );
    });
    ...
    test("Unknown YourEuropeCategoryType can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${conceptSnapshotId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#yourEuropeCategory> <https://productencatalogus.data.vlaanderen.be/id/concept/YourEuropeCatagory/NonExistingYourEuropeCategory> .' niet mappen.`,
      );
    });
    describe("isArchived", () => {
    ...
    test("Unknown ConceptTag Type can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan '<${conceptSnapshotId}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#conceptTag> <https://productencatalogus.data.vlaanderen.be/id/concept/ConceptTag/NonExistingConceptTag> .' niet mappen.`,
      );
    });
  });
});
