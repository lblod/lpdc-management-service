...
describe("FormalInformalChoiceRepository", () => {
  const repository = new FormalInformalChoiceSparqlRepository(
  ...
  describe("findByBestuurseenheid", () => {
    test("When formal informal choice exists for a bestuurseenheid, then return formal informal choice", async () => {
      ...
      const anotherFormalInformalChoice = aFormalInformalChoice()
        .withBestuurseenheidId(anotherBestuurseenheid.id)
        .build();
      await repository.save(
        anotherBestuurseenheid,
        anotherFormalInformalChoice,
      );

      const actualFormalInformalChoice =
        await repository.findByBestuurseenheid(bestuurseenheid);
    ...
    test("When multiple formal informal choices for a bestuurseenheid, then return oldest formal informal choice", async () => {
      ...
      const formalInformalChoiceSavedFirst = aFormalInformalChoice()
        .withBestuurseenheidId(bestuurseenheid.id)
      ...
      const formalInformalChoiceSavedAfter = aFormalInformalChoice()
        .withBestuurseenheidId(bestuurseenheid.id)
      ...
      expect(actualFormalInformalChoice).toEqual(
        formalInformalChoiceSavedFirst,
      );
    });
  });
  ...
  describe("Verify ontology and mapping", () => {
    test("Verify mappings", async () => {
      ...
      await directDatabaseAccess.insertData(
        ...
        [PREFIX.lpdc, PREFIX.mu, PREFIX.schema, PREFIX.dct],
      );
    ...
    test("formal informal choice in incorrect user graph throws error", async () => {
      ...
      await expect(
        repository.findByBestuurseenheid(bestuurseenheid),
      ).rejects.toThrowWithMessage(
        SystemError,
        `Formele informele keuze met id: <${formalInformalChoice.id}> gevonden in de foute gebruikers graph`,
      );
    });
    ...
    test("Unknown ChosenForm can not be mapped", async () => {
      ...
      await expect(
        ...
        SystemError,
        `Kan 'non-existing-choice'niet mappen voor Iri: <${formalInformalChoiceIri}>`,
      );
    });
  });
});
