...
describe("VersionedLdesSnapshotRepository", () => {
  ...
  const repository = new VersionedLdesSnapshotSparqlRepository(
    TEST_SPARQL_ENDPOINT,
  );
  ...
  describe("findToProcessInstanceSnapshots", () => {
    test("When no snapshots processed, then return all", async () => {
      ...
      const anotherInstanceSnapshotGraph = new Iri(
        INSTANCE_SNAPHOT_LDES_GRAPH("another-integrating-partner"),
      );
      ...
      const instanceSnapshot2 = aFullInstanceSnapshot()
        .withCreatedBy(bestuurseenheid.id)
        .build();
      const instanceSnapshotOtherBestuurseenheid = aFullInstanceSnapshot()
        .withCreatedBy(otherBestuurseenheid.id)
        .build();
      ...
      await instanceSnapshotRepository.save(
        anotherInstanceSnapshotGraph,
        instanceSnapshotOtherBestuurseenheid,
      );

      const actual = await repository.findToProcessSnapshots(
    ...
    test("When some snapshots are already processed, then return only to process instanceSnapshots", async () => {
      ...
      await repository.addToSuccessfullyProcessedSnapshots(
        instanceSnapshotGraph,
        instanceSnapshot2.id,
      );
    ...
    test("should return findToProcessInstanceSnapshots sorted by generatedAt date", async () => {
      ...
      const instanceSnapshot1 = aFullInstanceSnapshot()
        .withGeneratedAtTime(
          FormatPreservingDate.of("2024-01-05T00:00:00.657Z"),
        )
        .withCreatedBy(bestuurseenheid.id)
        .build();
      const instanceSnapshot2 = aFullInstanceSnapshot()
        .withGeneratedAtTime(
          FormatPreservingDate.of("2024-01-07T00:00:00.657Z"),
        )
        .withCreatedBy(bestuurseenheid.id)
        .build();
      const instanceSnapshotOtherBestuurseenheid = aFullInstanceSnapshot()
        .withGeneratedAtTime(
          FormatPreservingDate.of("2024-01-06T00:00:00.657Z"),
        )
        .withCreatedBy(otherBestuurseenheid.id)
        .build();
      await instanceSnapshotRepository.save(
  ...
  describe("addToSuccessfullyProcessedSnapshots", () => {
    test("should add to processedSnapshot with status success to given graph", async () => {
      ...
      expect(result[0]["dateCreated"]?.value).toEqual(
  ...
  describe("addToFailedProcessedSnapshots", () => {
    test("should add to processedSnapshot with status failed to given graph", async () => {
      ...
      await repository.addToFailedProcessedSnapshots(
        instanceSnapshotGraph,
        instanceSnapshotId,
        "error message received and to be saved",
      );
