...
import {
  ...
  extractAllConceptCodesForInstanceSnapshot,
...
describe("Instance Snapshot Data Integrity Validation", () => {
  ...
  test.skip(
    "Query for each bestuurseenheid the instance snapshots in the ldes input graph",
    async () => {
      ...
      try {
        ...
        const instanceSnapshotGraphAsStrings: string[] = sortedUniq(
          instanceSnapshots.map((is) => is.instanceSnapshotGraph.value),
        );
        ...
        for (const instanceSnapshotGraphStr of instanceSnapshotGraphAsStrings) {
          ...
          const instanceSnapshotIds = instanceSnapshots
            .filter(
              (is) =>
                is.instanceSnapshotGraph.value === instanceSnapshotGraph.value,
            )
            .map((is) => is.instanceSnapshotId);

          console.log(
            `verifying ${instanceSnapshotIds.length} instance snapshots in <${instanceSnapshotGraph}>`,
          );
          ...
          allQuadsOfGraph = allQuadsOfGraph.filter(
            (q) =>
              !q.predicate.equals(
                namedNode("http://mu.semte.ch/vocabularies/ext/processed"),
              ),
          );
          ...
          for (const instanceSnapshotId of instanceSnapshotIds) {
            try {
              const instanceSnapshot = await repository.findById(
                instanceSnapshotGraph,
                instanceSnapshotId,
              );
              expect(instanceSnapshot.id).toEqual(instanceSnapshotId);

              const quadsForInstanceSnapshotForId = new DomainToQuadsMapper(
                instanceSnapshotGraph,
              ).instanceSnapshotToQuads(instanceSnapshot);
              quadsForQueriedInstanceSnapshots = [
                ...quadsForInstanceSnapshotForId,
                ...quadsForQueriedInstanceSnapshots,
              ];
          ...
          const quadsFromRequeriedInstanceSnapshotsAsStrings =
            quadsForQueriedInstanceSnapshots.map((quad) => quad.toString());

          const allRemainingQuadsOfGraphAsTurtle = allQuadsOfGraph
            .map((q) => q.toString())
            .filter(
              (q) => !quadsFromRequeriedInstanceSnapshotsAsStrings.includes(q),
            );
  ...
  async function findAllInstanceSnapshots(): Promise<
    {
