...
describe('Instance informal language strings fetcher', () => {
    ...
    test.skip('Load all instances; fetch published ones to Ipdc and map to informal', async () => {
        ...
        for (const bestuurseenheidId of bestuurseenhedenIds) {
            ...
            const instanceIds: string[] = await getInstancesForBestuurseenheid(bestuurseenheid);
            console.log(`Verifying bestuurseenheid ${bestuurseenheidId} with ${instanceIds.length} instances`);
            ...
            if (instanceErrors.length != 0) {
                fs.writeFileSync(`/tmp/failing-informals/${bestuurseenheid.uuid}.txt`, sortedUniq(instanceErrors).join('\n'));
            }
...
async function getInstancesForBestuurseenheid(bestuurseenheid: Bestuurseenheid): Promise<string[]> {
    const query = `
            ${PREFIX.lpdcExt}
            SELECT ?id WHERE {
