...
describe("Concept Display Configuration Data Integrity Validation", () => {
  ...
  const conceptDisplayConfigurationRepository =
    new ConceptDisplayConfigurationSparqlTestRepository(endPoint);
  const directDatabaseAccess = new DirectDatabaseAccess(endPoint);

  test.skip(
    "Load all concept display configurations; print errors to console.log",
    async () => {
      const query = `
            ${PREFIX.besluit}
            SELECT ?id WHERE {
      ...
      console.log(
        `Verifying Concept Display Configurations for ${bestuurseenheidIdsResult.length} bestuurseenheden`,
      );
      ...
      console.log(
        `Verifying Concept Display Configurations for ${conceptIdsResult.length} concepts`,
      );
      ...
      for (const bestuursEenheidResult of bestuurseenheidIdsResult) {
        try {
          ...
          for (const conceptResult of conceptIdsResult) {
            ...
            expect(
              `Concept display configuration found for ${conceptDisplayConfiguration?.conceptId} - ${conceptDisplayConfiguration?.bestuurseenheidId}`,
            ).toEqual(
              `Concept display configuration found for ${conceptId} - ${bestuurseenheid.id}`,
            );
          }
        } catch (e) {
          console.log(e);
      ...
      const totalAverageTime =
        (new Date().valueOf() - before) /
        (bestuurseenheidIdsResult.length * conceptIdsResult.length);
