...
describe("Formal Informal Choice Data Integrity Validation", () => {
  ...
  test.skip(
    "Load all formal informal choice; print errors to console.log",
    async () => {
      const query = `
            ${PREFIX.besluit}
            SELECT ?id WHERE {
      ...
      console.log(
        `Verifying Formal Informal Choice for ${bestuurseenheidIdsResult.length} bestuurseenheden`,
      );
      ...
      for (const bestuurseenheidResult of bestuurseenheidIdsResult) {
        try {
          ...
          if (formalInformalChoice) {
            expect(formalInformalChoice.bestuurseenheidId).toEqual(
              bestuurseenheid.id,
            );
          }
        } catch (e) {
      ...
      const totalAverageTime =
        (new Date().valueOf() - before) / bestuurseenheidIdsResult.length;
