...
describe("fully take concept snapshot over", () => {
  test.skip(
    "Load all published instances; and verify ",
    async () => {
      ...
      for (const bestuurseenheidId of bestuurseenhedenIds) {
        ...
        const instanceIds: string[] =
          await getInstancesWithHerzieningNodigForBestuurseenheid(
            bestuurseenheid,
          );
        console.log(
          `Verifying bestuurseenheid ${bestuurseenheidId} with ${instanceIds.length} herziening nodig instances`,
        );
        ...
        if (instanceErrors.length != 0) {
          fs.writeFileSync(
            `/tmp/failing-fully-take-concept-snapshot-over/${bestuurseenheid.uuid}.txt`,
...
async function getInstancesWithHerzieningNodigForBestuurseenheid(
