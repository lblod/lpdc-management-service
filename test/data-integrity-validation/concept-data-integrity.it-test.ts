...
import {
  ConceptCodeValidator,
  extractAllConceptCodesForConcept,
} from "./helpers/concept-code.validator";
import fs from "fs";
...
describe("Concept Data Integrity Validation", () => {
  ...
  test.skip(
    "Load all concepts; print errors to console.log",
    async () => {
      ...
      const allTriplesOfGraph = await directDatabaseAccess.list(
        allTriplesOfGraphQuery,
      );
      let allQuadsOfGraph: Statement[] = uniq(
        sparqlQuerying.asQuads(allTriplesOfGraph, conceptGraph.value),
      );

      //filter out all triples linked to account subjects
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith("http://data.lblod.info/id/account/"),
      );

      //filter out all triples linked to adressen subjects
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith("http://data.lblod.info/id/adressen/"),
      );

      //filter out all triples linked to persoon subjects
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith("http://data.lblod.info/id/persoon/"),
      );

      //filter out all triples linked to bestuurseenheden subjects
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith(
      ...
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith(
            "http://data.lblod.info/id/werkingsgebieden/",
          ),
      );
      ...
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith(
            "http://data.lblod.info/werkingsgebieden/id/",
          ),
      );
      ...
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith(
            "http://data.vlaanderen.be/id/concept/BestuurseenheidClassificatieCode/",
          ),
      );
      ...
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) => !q.subject.value.startsWith("http://lblod.data.gift/concepts/"),
      );
      ...
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith(
            "https://productencatalogus.data.vlaanderen.be/id/concept/",
          ),
      );
      ...
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith(
            "https://productencatalogus.data.vlaanderen.be/id/conceptscheme/",
          ),
      );
      ...
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith(
            "https://data.vlaanderen.be/id/organisatie/",
          ),
      );

      //filter out all triples linked to http://data.europa.eu/nuts/code/ subjects
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) => !q.subject.value.startsWith("http://data.europa.eu/nuts/code/"),
      );

      //filter out all triples linked to http://data.europa.eu/nuts/scheme/2021 concept scheme
      allQuadsOfGraph = allQuadsOfGraph.filter(
        (q) =>
          !q.subject.value.startsWith("http://data.europa.eu/nuts/scheme/2021"),
      );
      ...
      for (const result of randomizedConceptIds) {
        ...
        try {
          ...
          quadsFromRequeriedConcepts = [
            ...quadsForConceptForId,
            ...quadsFromRequeriedConcepts,
          ];

          await conceptCodeValidator.validateConceptCodes(
            extractAllConceptCodesForConcept(domainToQuadsMapper, conceptForId),
          );

          if (alsoLoadRelatedConceptSnapshots) {
            const latestConceptSnapshot = await snapshotRepository.findById(
              conceptForId.latestConceptSnapshot,
            );
            expect(latestConceptSnapshot.id).toEqual(
              conceptForId.latestConceptSnapshot,
            );
            ...
            for (const eachPreviousConceptSnapshotId of conceptForId.previousConceptSnapshots) {
              const previousConceptSnapshot = await snapshotRepository.findById(
                eachPreviousConceptSnapshotId,
              );
              expect(previousConceptSnapshot.id).toEqual(
                eachPreviousConceptSnapshotId,
              );
              expect(previousConceptSnapshot.isVersionOf).toEqual(id);
            }

            const latestFunctionallyChangedConceptSnapshot =
              await snapshotRepository.findById(
                conceptForId.latestFunctionallyChangedConceptSnapshot,
              );
            expect(latestFunctionallyChangedConceptSnapshot.id).toEqual(
              conceptForId.latestFunctionallyChangedConceptSnapshot,
            );
            expect(
              latestFunctionallyChangedConceptSnapshot.isVersionOf,
            ).toEqual(id);

            validateThatConceptDataIsInSyncWithLatestConceptSnapshot(
              conceptForId,
              latestConceptSnapshot,
              latestFunctionallyChangedConceptSnapshot,
            );
          }
        } catch (e) {
          console.error(
            `Error while verifying concept ${sparqlEscapeUri(id)}`,
            e,
          );
      ...
      const quadsFromRequeriedConceptsAsStrings =
        quadsFromRequeriedConcepts.map((quad) => quad.toString());
      ...
      fs.writeFileSync(
        `/tmp/remaining-quads-concept.txt`,
        sortedUniq(allRemainingQuadsOfGraphAsTurtle).join("\n"),
      );
      //expect(sortedUniq(allRemainingQuadsOfGraphAsTurtle)).toEqual([]);

      const averageTime =
        ((new Date().valueOf() - before) * conceptIds.length) /
        conceptIds.length;
      averageTimes.push(averageTime);

      console.log(
        `Verifying in total ${conceptIds.length} concept took on average ${averageTime} ms per concept`,
      );
      // eslint-disable-next-line no-constant-condition

      const totalAverageTime =
        averageTimes.reduce((accumulator, currentValue) => {
  ...
  test.skip("Load one concept and print quads", async () => {
    const id = new Iri(
      "https://ipdc.vlaanderen.be/id/concept/26bfa5c8-f099-44c8-8765-37b1ab095850",
    );
    const fetcher = new DatastoreToQuadsRecursiveSparqlFetcher(endPoint);

    const allQuads = await fetcher.fetch(
      conceptGraph,
      id,
    ...
    const allConceptsToQuadsAsStrings = asSortedArray(
      conceptToQuads.map((q) => q.toString()),
    );
  ...
  function validateThatConceptDataIsInSyncWithLatestConceptSnapshot(
    concept: Concept,
    latestConceptSnapshot: ConceptSnapshot,
    latestFunctionallyChangedConceptSnapshot: ConceptSnapshot,
  ) {
    expect(
      isConceptFunctionallyChangedComparedToConceptSnapshot(
        concept,
        latestFunctionallyChangedConceptSnapshot,
      ),
    ).toBeFalsy();
  ...
  function isConceptFunctionallyChangedComparedToConceptSnapshot(
    value: Concept,
