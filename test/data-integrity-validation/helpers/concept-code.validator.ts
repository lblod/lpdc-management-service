...
export function extractAllConceptCodesForInstance(domainToQuadsMapper: DomainToQuadsMapper, instance: Instance): Iri[] {
    return [
...
export function extractAllConceptCodesForInstanceSnapshot(domainToQuadsMapper: DomainToQuadsMapper, instanceSnapshot: InstanceSnapshot): Iri[] {
    return [
        ...extractAllConceptCodes(domainToQuadsMapper, instanceSnapshot),
        ...domainToQuadsMapper.languages(instanceSnapshot.id, instanceSnapshot.languages).map(objectAsIri),
        ...instanceSnapshot.spatials,
    ];
}

export function extractAllConceptCodesForConceptSnapshot(domainToQuadsMapper: DomainToQuadsMapper, conceptSnapshot: ConceptSnapshot): Iri[] {
    return [
        ...extractAllConceptCodes(domainToQuadsMapper, conceptSnapshot),
        ...domainToQuadsMapper.conceptTags(conceptSnapshot.id, conceptSnapshot.conceptTags).map(objectAsIri),
        ];
}

export function extractAllConceptCodesForConcept(domainToQuadsMapper: DomainToQuadsMapper, concept: Concept): Iri[] {
    return [
        ...extractAllConceptCodes(domainToQuadsMapper, concept),
        ...domainToQuadsMapper.conceptTags(concept.id, concept.conceptTags).map(objectAsIri),
    ];
}

function extractAllConceptCodes(domainToQuadsMapper: DomainToQuadsMapper, value: Instance | ConceptSnapshot | Concept | InstanceSnapshot): Iri[] {
    return [
        ...(value.type ? [domainToQuadsMapper.type(value.id, value.type)].map(objectAsIri): []),
        ...domainToQuadsMapper.targetAudiences(value.id, value.targetAudiences).map(objectAsIri),
        ...domainToQuadsMapper.themes(value.id, value.themes).map(objectAsIri),
        ...domainToQuadsMapper.competentAuthorityLevels(value.id, value.competentAuthorityLevels).map(objectAsIri),
        ...domainToQuadsMapper.executingAuthorityLevels(value.id, value.executingAuthorityLevels).map(objectAsIri),
        ...domainToQuadsMapper.publicationMedia(value.id, value.publicationMedia).map(objectAsIri),
        ...domainToQuadsMapper.yourEuropeCategories(value.id, value.yourEuropeCategories).map(objectAsIri),
    ];
}
