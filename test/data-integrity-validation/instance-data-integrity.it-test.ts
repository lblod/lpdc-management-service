...
import {
  ...
  extractAllConceptCodesForInstance,
...
class DoubleQuadReporterCapture implements DoubleQuadReporter {
  ...
  report(
    ...
    this._logs.push(
      `${this._bestuurseenheid.id.value}|${this._bestuurseenheid.prefLabel}|INSTANCE_MARKER|${subject}|${predicate}|${object}|${expectedCount}|${actualCount}|${triples.join("|")}`,
    );
  }

  logs(instance: Instance): string[] {
    return this._logs.map((str) =>
      str.replace("INSTANCE_MARKER", `${instance.id}|${instance.title?.nl}`),
    );
  }
}
...
describe("Instance Data Integrity Validation", () => {
  ...
  const informalFormalChoiceRepository =
    new FormalInformalChoiceSparqlRepository(endPoint);

  test.skip(
    "Load all instances; print errors to console.log",
    async () => {
      ...
      const randomizedBestuurseenheidsIds = shuffle([
        ...bestuurseenheidIdsResult,
      ]);
      ...
      console.log(
        `Verifying ${randomizedBestuurseenheidsIds.length} bestuurseenheden`,
      );
      ...
      for (const bestuurseenheidId of randomizedBestuurseenheidsIds) {
        const bestuurseenheid = await bestuurseenheidRepository.findById(
          new Iri(bestuurseenheidId["id"].value),
        );
        const domainToQuadsMapper = new DomainToQuadsMapper(
        ...
        const tombStonesSubjects = new Set([
          ...allQuadsOfGraph
            .filter((q) =>
              q.object.equals(
        ...
        allQuadsOfGraph = allQuadsOfGraph.filter(
          (q) => !tombStonesSubjects.has(q.subject.value),
        );
        ...
        allQuadsOfGraph = allQuadsOfGraph.filter(
          (q) =>
            !q.subject.value.startsWith(
              "http://data.lblod.info/id/conceptual-display-configuration/",
            ),
        );
        allQuadsOfGraph = allQuadsOfGraph.filter(
          (q) =>
            !q.predicate.equals(
              namedNode(
                "http://data.lblod.info/vocabularies/lpdc/hasConceptDisplayConfiguration",
              ),
            ),
        );
        allQuadsOfGraph = allQuadsOfGraph.filter(
          (q) =>
            !q.subject.value.startsWith(
              "http://data.lblod.info/id/formalInformalChoice/",
            ),
        );
        ...
        if (instanceIds.length > 0) {
          for (let i = 0; i < numberOfLoops; i++) {
            ...
            for (const instanceId of randomizedInstanceIds) {
              try {
                const doubleQuadReporterCapture = new DoubleQuadReporterCapture(
                  bestuurseenheid,
                );
                const repository = new InstanceSparqlRepository(
                  endPoint,
                  doubleQuadReporterCapture,
                );
                ...
                if (instance.conceptId) {
                  const concept = await conceptRepository.findById(
                    instance.conceptId,
                  );
                  await conceptSnapshotRepository.findById(
                ...
                await validateNeedsConversionFromFormalToInformalFlag(
                  instance,
                  bestuurseenheid,
                );
                ...
                const doubleQuadsErrorsForInstance =
                  doubleQuadReporterCapture.logs(instance);
            ...
            const quadsFromRequeriedInstancesAsStrings =
              quadsFromRequeriedInstances.map((quad) => quad.toString());
            ...
            if (allRemainingQuadsOfGraphAsTurtle.length > 0) {
              fs.writeFileSync(
                `/tmp/remaining-quads-instance/remaining-quads-instance-${bestuurseenheid.uuid}.txt`,
                sortedUniq(allRemainingQuadsOfGraphAsTurtle).join("\n"),
              );
              totalRemainingQuadsInstance.push(
                ...allRemainingQuadsOfGraphAsTurtle,
              );
            }
            ...
            const averageTime =
              (new Date().valueOf() - before - delayTime * instanceIds.length) /
              instanceIds.length;
          ...
          if (dataErrors.length > 0) {
            console.log(
              `Data Errors [${dataErrors.map((o) => JSON.stringify(o))}]`,
            );
            totalErrors.push(dataErrors);
            console.log(
              "totalErrors" + totalErrors.map((o) => JSON.stringify(o)),
            );
          }
      ...
      fs.writeFileSync(
        `/tmp/instance-total-errors.json`,
        sortedUniq(totalErrors.map((o) => JSON.stringify(o))).join("\n"),
      );
      fs.writeFileSync(
        `/tmp/instance-total-double-triples.csv`,
        totalDoubleQuads.join("\n"),
      );
      fs.writeFileSync(
        `/tmp/instance-total-remaining-quads.txt`,
        totalRemainingQuadsInstance.join("\n"),
      );
  ...
  test.skip("Find all triples for instance", async () => {
    ...
    const instanceId = InstanceBuilder.buildIri(
      "38c47504-7e2c-4290-a2fb-778b7f5ca05c",
    );
    const triples = await getInstanceTriples(
      endPoint,
      bestuurseenheid.userGraph(),
      instanceId,
    );
    ...
    instance.id;
    const domainToTriplesMapper = new DomainToQuadsMapper(
      bestuurseenheid.userGraph(),
    );
  ...
  async function getInstanceTriples(
    endpoint: string,
    bestuurseenheidUserGraph: Iri,
    instanceUUID: Iri,
  ): Promise<string[]> {
    ...
    const allQuadsAsStrings: string[] = asSortedArray(
  ...
  async function validateNeedsConversionFromFormalToInformalFlag(
    instance: Instance,
    bestuurseenheid: Bestuurseenheid,
  ) {
    const formalInformalChoice =
      await informalFormalChoiceRepository.findByBestuurseenheid(
        bestuurseenheid,
      );
