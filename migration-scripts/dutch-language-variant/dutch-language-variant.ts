...
async function main() {
  ...
  for (const bestuurseenheidId of bestuurseenhedenIds) {
    ...
    for (const instanceId of instanceIds) {
      ...
      const dutchLanguageVersion = await calculateDutchLanguageVariant(
        bestuurseenheid,
        instance,
      );
      const dutchLanguageVersionQuads = dutchLanguageVariantToQuad(
        instance,
        bestuurseenheid,
        dutchLanguageVersion,
      );
  ...
  fs.writeFileSync(
    `./migration-results/insertDutchLanguageVariant.ttl`,
    insertQuads.join("\n"),
  );
...
async function calculateDutchLanguageVariant(
  bestuurseenheid: Bestuurseenheid,
  instance: Instance,
): Promise<Language> {
  ...
  return formalInformalChoice?.chosenForm === ChosenFormType.INFORMAL
    ? Language.INFORMAL
    : Language.FORMAL;
}

function dutchLanguageVariantToQuad(
  instance: Instance,
  bestuurseenheid: Bestuurseenheid,
  dutchLanguageVariant: string,
): string {
