...
const graphId = new Iri(
  "http://mu.semte.ch/graphs/organizations/f4a187c72e551b9d7745e3b8602b11f12ce0fd5399c7f8aebbd4f8f42dc9c028/LoketLB-LPDCGebruiker",
);
...
async function fetchIpdcInstance() {
  ...
  const oldInstancequads = await fetchExistingTriples(
    bestuurseenheid,
    instanceIri,
  );

  fs.writeFileSync(
    `./migration-results/delete-corrupted-instance-${instanceUuid}.sparql`,
    `DELETE DATA {
  ...
  const instance = await mapInstance(
  ...
  const publishedInstanceQuads =
    domainToQuadsMapper.publishedInstanceSnapshotToQuads(
      publishedInstanceSnapshot,
    );
  const quads = [...instanceQuads, ...publishedInstanceQuads];
  fs.writeFileSync(
    `./migration-results/restore-corrupted-instance-ipdc-${instanceUuid}.ttl`,
    quads.map((quad) => quad.toCanonical()).join("\n"),
  );
}
...
async function fetchInstance(id: Iri, uuid: string) {
  ...
  if (response.status === 401) {
    ...
    throw new SystemError(
      `Er is een fout opgetreden bij het bevragen van Ipdc voor instantie ${uuid}; status=[${response.status}]`,
    );
  }
}
        ...
        (error: any, kb: any) => {
          ...
          const uuids = uniq(
            ...
            (blankNode) =>
              new Statement(
                blankNode,
                new NamedNode(NS.mu("uuid").value),
                new Literal(uuid()),
                new NamedNode(graphId.value),
              ),
          );
...
async function toInstance(
  ...
  const concept = await conceptRepository.findById(
    mapper.conceptId(instanceIri),
  );
  if (!mapper.createdBy(instanceIri).equals(bestuurseenheid.id)) {
    throw new Error(
      `createdby (${mapper.createdBy(instanceIri)}) from ipdc does not match bestuurseenheid ${bestuurseenheid.id}`,
    );
  }

  const contactPoints = await mapContactpoints(
    mapper.contactPoints(instanceIri),
  );
  ...
  return new InstanceBuilder()
    ...
    .withTitle(
      mapper.title(instanceIri)?.transformLanguage(Language.NL, chosenLanguage),
    )
    .withDescription(
      mapper
        .description(instanceIri)
        ?.transformLanguage(Language.NL, chosenLanguage),
    )
    .withAdditionalDescription(
      mapper
        .additionalDescription(instanceIri)
        ?.transformLanguage(Language.NL, chosenLanguage),
    )
    .withException(
      mapper
        .exception(instanceIri)
        ?.transformLanguage(Language.NL, chosenLanguage),
    )
    .withRegulation(
      mapper
        .regulation(instanceIri)
        ?.transformLanguage(Language.NL, chosenLanguage),
    )
    ...
    .withWebsites(
      mapper
        .websites(instanceIri)
        .map((w) =>
          w.transformLanguage(Language.NL, chosenLanguage).transformWithNewId(),
        ),
    )
    .withCosts(
      mapper
    ...
    .withFinancialAdvantages(
      mapper
        .financialAdvantages(instanceIri)
        .map((fa) =>
          fa
            .transformLanguage(Language.NL, chosenLanguage)
            .transformWithNewId(),
    ...
    .withLegalResources(
      mapper
        .legalResources(instanceIri)
        .map((lr) =>
          lr
            .transformLanguage(Language.FORMAL, chosenLanguage)
            .transformWithNewId(),
        ),
    )
...
async function mapContactpoints(
...
async function addAdressId(address: Address): Promise<Address> {
  ...
  if (match) {
    return AddressBuilder.from(address)
      .withVerwijstNaar(new Iri(match["adressenRegisterId"]))
      .build();
  }
  return address;
}

async function fetchExistingTriples(
  bestuurseenheid: Bestuurseenheid,
  instanceId: Iri,
) {
