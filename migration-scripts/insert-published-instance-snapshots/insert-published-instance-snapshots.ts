...
async function main() {
  ...
  for (const bestuurseenheidId of bestuurseenhedenIds) {
    ...
    const instanceIds =
      await getAllPublishedInstanceIdsForBestuurseenheidWithoutPublishedSnapshot(
        bestuurseenheid,
      );

    const segmentedBestuurseenheidId = bestuurseenheidId.value.split("/");
    const uuidExtractedFromBestuurseenheidId =
      segmentedBestuurseenheidId[segmentedBestuurseenheidId.length - 1];
    ...
    for (const { id: instanceId, datePublished } of instanceIds) {
      ...
      if (
        ...
        const publishedInstanceSnapshotTriples = new DomainToQuadsMapper(
          bestuurseenheid.userGraph(),
        ).publishedInstanceSnapshotToQuads(publishedInstanceSnapshot);

        insertQuads.push(
          ...publishedInstanceSnapshotTriples.map((quad) => quad.toNT()),
        );
        insertQuads.push(
          `<${publishedInstanceSnapshot.id.value}> <http://schema.org/datePublished> ${sparqlEscapeDateTime(datePublished.value)}.`,
        );
        ...
        if (insertQuads.length > 1000) {
          fs.writeFileSync(
            `./migration-results/${baseFileName}-${sequenceNumber}.sparql`,
            `INSERT DATA {
  ...
  const deleteDatePublishedInstances = `
            ...
            `;
  fs.writeFileSync(
    `./migration-results/${now()}-delete-date-published-instances.sparql`,
    deleteDatePublishedInstances,
  );
  ...
  for (const bestuurseenheidId of bestuurseenhedenIds) {
    const bestuurseenheid =
      await bestuurseenheidRepository.findById(bestuurseenheidId);
    const tombstoneIds =
      await getAllPublishedTombstonesForBestuurseenheid(bestuurseenheid);
    ...
    for (const { id: instanceId, dateDeleted, datePublished } of tombstoneIds) {
      ...
      insertQuads.push(
        `<${tombstoneId.value}> a <https://www.w3.org/ns/activitystreams#Tombstone>.`,
      );
      insertQuads.push(
        `<${tombstoneId.value}> <https://www.w3.org/ns/activitystreams#formerType> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#InstancePublicService>.`,
      );
      insertQuads.push(
        `<${tombstoneId.value}> <https://productencatalogus.data.vlaanderen.be/ns/ipdc-lpdc#isPublishedVersionOf> <${instanceId.value}>.`,
      );
      insertQuads.push(
        `<${tombstoneId.value}> <https://www.w3.org/ns/activitystreams#deleted>  ${sparqlEscapeDateTime(dateDeleted.value)} .`,
      );
      insertQuads.push(
        `<${tombstoneId.value}> <http://www.w3.org/ns/prov#generatedAtTime>  ${sparqlEscapeDateTime(dateDeleted.value)} .`,
      );
      insertQuads.push(
        `<${tombstoneId.value}> <http://schema.org/datePublished>  ${sparqlEscapeDateTime(datePublished.value)} .`,
      );
    ...
    if (insertQuads.length > 0) {
      fs.writeFileSync(
        `./migration-results/${baseFileName}.sparql`,
        `INSERT DATA {
                            GRAPH ${sparqlEscapeUri(bestuurseenheid.userGraph())} {
                                ${insertQuads.join("\n")}
                            }
  ...
  const deleteTombstoneInstances = `
            ...
            `;
  fs.writeFileSync(
    `./migration-results/${now()}-delete-tombstone-instances.sparql`,
    deleteTombstoneInstances,
  );
}
...
async function getAllBestuurseenheden(): Promise<Iri[]> {
  ...
  return bestuurseenheden.map(
    (bestuurseenheid) => new Iri(bestuurseenheid["id"].value),
  );
}

async function getAllPublishedInstanceIdsForBestuurseenheidWithoutPublishedSnapshot(
  bestuurseenheid: Bestuurseenheid,
): Promise<InstanceAndDatePublished[]> {
  const query = `
            ${PREFIX.lpdcExt}
            ${PREFIX.schema}
...
async function getAllPublishedTombstonesForBestuurseenheid(
  bestuurseenheid: Bestuurseenheid,
